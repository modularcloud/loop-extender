...
1. **Dependency management:** The workflow can declare its own dependencies. `loopx install` auto-runs `npm install` in each committed workflow whose top-level `package.json` declares dependencies (see section 10.10), so a workflow's `node_modules/` is normally populated by the time `loopx run` first spawns one of its scripts. The `--no-install` flag on `loopx install` suppresses the install-time step for users who prefer manual control or a non-npm package manager. At runtime, loopx does **not** re-install dependencies — `loopx run` does not invoke `npm install` on a missing `node_modules/`. If `node_modules/` is missing and a script fails to import a package, the resulting error is the active runtime's normal module resolution error.
...
The project root is also the script execution cwd for every spawned script (see section 6.1) and the resolution base for the script-execution `LOOPX_WORKFLOW_DIR` injection (see section 6.1 and section 8). For `RunOptions.cwd` type and validation rules, see section 9.5.
...
- A sole positional `name=value` (e.g., `loopx run adr=0003`) is parsed as a target string and fails target validation because `=` is not in the `[a-zA-Z0-9_][a-zA-Z0-9_-]*` pattern. If `.loopx/` discovery has not yet established the failure path, target validation surfaces the rejection.
...
**Environment variables are loaded once at loop start and cached for the duration of the loop.** Changes to env files during loop execution are not picked up until the next invocation. The same once-per-run rule extends to the inherited `process.env`: loopx snapshots the caller's `process.env` once during the pre-iteration sequence and reuses the snapshot for every child spawn in the run; mutations to `process.env` between spawns do not propagate. The tmpdir parent (the `TMPDIR` / `TEMP` / `TMP` values that `os.tmpdir()` reads in loopx's own process) is captured on the same schedule as the inherited-env snapshot — see section 7.4. Global env file path resolution (`$XDG_CONFIG_HOME/loopx/env`, with the documented `HOME`-based fallback) reads `XDG_CONFIG_HOME` / `HOME` from the inherited environment on the same schedule. Snapshot timing for the programmatic API (lazy under `run()`, eager under `runPromise()`) is specified in sections 9.1 and 9.2.
...
- When present and not `undefined`, must be an `AbortSignal`-compatible object exposing a readable `aborted` property and an `addEventListener('abort', listener)` method that is callable and returns without throwing when loopx invokes it to register its abort listener.
...
- Counts every target execution, including goto hops. `maxIterations: 0` mirrors CLI `-n 0` behavior: validates and exits without executing any iterations. No `LOOPX_TMPDIR` is created under `maxIterations: 0`.
...
- **Lifetime.** Entries are captured synchronously at call time as a shallow copy — loopx reads the supplied object's own enumerable string-keyed properties once. Mutating the original object after `run()` / `runPromise()` returns has no effect on the running loop. Any exception raised during the snapshot (a `Proxy` `ownKeys` trap that throws, a throwing enumerable getter, a throwing `options.env` getter on the options object) is captured and surfaced through the standard pre-iteration error path; it never escapes at the call site.
...
- **Install failure cleanup (pre-commit failure modes):** Any pre-commit install failure (download error, HTTP non-2xx, git clone failure, extraction failure, post-download validation failure) or staging-phase failure under section 10.7 exits with code 1. Any partially created staging directory is removed before exit. For single-workflow installs, any partially created target directory at the destination path is also removed before exit. This cleanup contract applies **only** to the pre-commit failure modes enumerated here. Post-commit auto-install failures specified in section 10.10 (`.gitignore` write failure, `npm install` non-zero exit, `npm install` spawn failure) still cause exit `1` but do **not** remove already-committed workflow directories from `.loopx/` and do **not** remove a partial `node_modules/` left behind by an interrupted install; those behaviors are governed by section 10.10's aggregate-report / no-rollback rule.
...
- **`.gitignore` safeguard for `node_modules/`.** Immediately before spawning `npm install` in a workflow directory, loopx checks for a top-level `.gitignore` in that workflow directory. If none exists, loopx creates one containing the single line `node_modules`. If a `.gitignore` already exists, loopx leaves it unchanged — the workflow author is responsible for the contents of a pre-existing `.gitignore`, and loopx does not append, rewrite, or warn about a missing `node_modules` entry. The safeguard runs under the same trigger conditions as `npm install`: once per committed workflow with a top-level `package.json`, skipped for workflows without one, skipped under the malformed-`package.json` skip below, and skipped under `--no-install`.
- **`.gitignore` write failure skips `npm install`.** A failure to create `.gitignore` (e.g., a read-only workflow directory) **skips `npm install` for that workflow** — loopx does not run `npm install` when the safeguard could not be established, since populating `node_modules/` without a covering `.gitignore` is exactly the hazard the safeguard is designed to prevent. The `.gitignore` failure is recorded on the same aggregate-report footing as an `npm install` spawn failure, loopx proceeds to the next workflow, and the failure contributes to the final exit `1`. Committed workflow files are not rolled back; the user can remediate by creating a `.gitignore` manually and running `cd .loopx/<workflow> && npm install`, or by re-running `loopx install -y <source>`.
...
| 1 | Error: script exited non-zero, validation failure, invalid `goto` target, missing workflow, missing script, missing `.loopx/` directory, `LOOPX_TMPDIR` creation failure, child launch / spawn failure after tmpdir creation, install failure (including `npm install` non-zero exit or spawn failure under section 10.10, and `.gitignore` safeguard write failure under section 10.10), or usage error. |
