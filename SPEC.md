...
**Environment variables are loaded once at loop start and cached for the duration of the loop.** Changes to env files during loop execution are not picked up until the next invocation. The same once-per-run rule extends to the inherited `process.env`: loopx snapshots the caller's `process.env` once during the pre-iteration sequence and reuses the snapshot for every child spawn in the run; mutations to `process.env` between spawns do not propagate. The tmpdir parent (the `TMPDIR` / `TEMP` / `TMP` values that `os.tmpdir()` reads in loopx's own process) is captured on the same schedule as the inherited-env snapshot — see section 7.4. Global env file path resolution (`$XDG_CONFIG_HOME/loopx/env`, with the documented `HOME`-based fallback) reads `XDG_CONFIG_HOME` / `HOME` from the inherited environment on the same schedule. `RunOptions.env` does **not** affect global env-file path resolution — `env` merges into the child environment after env-file loading (section 8.3), so an `XDG_CONFIG_HOME` or `HOME` entry supplied via `RunOptions.env` reaches spawned scripts but does not redirect where loopx looks for the global env file. Snapshot timing for the programmatic API (lazy under `run()`, eager under `runPromise()`) is specified in sections 9.1 and 9.2.
...
- **Aggregate failure report.** After the post-commit auto-install pass finishes, if any `.gitignore` synthesis or `npm install` failed for any workflow, loopx emits a final aggregate failure report to stderr that lists each affected workflow together with the underlying failure (`.gitignore` write failure, `npm install` non-zero exit, or `npm install` spawn failure) and then exits `1`. Workflows whose safeguard and `npm install` both succeeded are not listed. If no auto-install failures occurred, no aggregate report is emitted. The aggregate report does not roll back committed workflow files or any partial `node_modules/` left on disk; remediation is the manual procedure described in the preceding bullet and in the `.gitignore` write-failure bullet.
