...
- **Non-regular `package.json` path** (the entry at the workflow `package.json` path is a directory, symlink, FIFO, socket, or other non-regular entry, as observed via `lstat`): A warning is printed to stderr. The version check is skipped. Symlinks at the `package.json` path are not followed. Execution / installation proceeds. During `loopx install`, install-source symlinks named `package.json` are first governed by the install-source symlink rules in section 10.11. The non-regular `package.json` path rules above apply to runtime / already-committed workflow entries and to the materialized committed entry produced by install-source processing; they do not override section 10.11's source-symlink rejection / materialization rules.
...
- **Install:** A workflow's `package.json` may be evaluated at more than one point during an install operation: during source/preflight version validation (§10.6), and during the post-commit auto-install pass over the committed workflow (§10.10). Warning emission remains deduplicated: at most one `package.json` failure warning is emitted per affected workflow per install operation. `package.json` failure warnings (unreadable, invalid JSON, invalid semver range) do not block installation — the workflow is still installable, just without version validation. Version *mismatches* (a valid range not satisfied by the running version) are blocking errors and are included in the aggregated preflight failure report (see section 10.7).
...
- **Environment.** The `npm install` child inherits loopx's own `process.env` unchanged. `LOOPX_*` protocol variables, `RunOptions.env`, and env-file entries are not injected — these are script-execution-context concerns, not install-time concerns. `loopx install` does not read loopx env files (global or local). The §8.1 unreadable-global-env failure applies only to surfaces that read the env store (`loopx run`, `run()` / `runPromise()`, and `loopx env *` commands); `loopx install` is unaffected by an unreadable or malformed global env file, although ordinary inherited environment variables are still passed through unchanged to the `npm install` child.
...
- **`.gitignore` safeguard for `node_modules/`.** Immediately before spawning `npm install` in a workflow directory, loopx checks the top-level `.gitignore` path using `lstat`.
  ...
  - **`lstat` failure other than `ENOENT`, or a write failure when synthesizing `.gitignore`:** loopx treats this as a `.gitignore` safeguard failure under the same aggregate-report / skip-`npm install` semantics and, after observing the failure, makes no further changes to the `.gitignore` path for that workflow. In particular, loopx does not attempt to recover by creating, chmodding, unlinking, replacing, appending to, or rewriting the entry. Any partial filesystem effect already produced by a failed write is left for user remediation under this section's no-rollback rules.
...
- **Malformed `package.json`.** For auto-install, loopx evaluates the committed `.loopx/<workflow-name>/package.json` entry at the time that workflow is considered in the post-commit auto-install pass. This dispatch uses the committed filesystem state at that moment, not solely a source/preflight-cached validation result. If the committed `package.json` path is absent at that time, the workflow is skipped as a workflow without a top-level `package.json`. If the committed path exists but is unreadable, contains invalid JSON, has an invalid `loopx` semver range, or is at a non-regular path (directory, symlink, FIFO, socket, or other non-regular entry — the section 3.2 failure modes), the existing section 3.2 warning is emitted if one has not already been emitted for that workflow during this install operation, and auto-install **skips that workflow silently** — loopx does not invoke `npm install` against a file that failed version validation and does not add a second warning for the same underlying failure. A valid `loopx` range not satisfied by the running version remains a §10.6 preflight concern; if the workflow was committed because `-y` overrode that mismatch, the valid mismatch is not a malformed-`package.json` auto-install skip condition. The `.gitignore` safeguard is also skipped for the same workflow. A skipped workflow is not an auto-install failure and does not appear in the aggregate failure report. If an install-source `package.json` symlink was materialized to a regular file under section 10.11, the auto-install pass treats the committed regular file normally; it is not skipped merely because the original source entry was a symlink. If section 10.11 rejected a broken, cyclic, or out-of-source source `package.json` symlink, the workflow is not committed and this post-commit auto-install skip path is not reached.
...
**`package.json` source symlink precedence.** A source entry named `package.json` is not special for the install-source symlink rules in this section. When a selected workflow's source `package.json` entry is a symlink, this section's symlink validation and materialization rules apply before the workflow-`package.json` failure-mode dispatch in section 3.2 / section 10.10. A source `package.json` symlink that resolves to an existing in-source regular file is materialized as a regular committed `package.json`; install-time version validation and post-commit auto-install then treat it as a regular `package.json`. A source `package.json` symlink that resolves to an in-source directory is materialized as a directory and then falls under the non-regular `package.json` path behavior in section 3.2 / section 10.10. Broken, cyclic, or out-of-source `package.json` symlinks, or `package.json` symlinks resolving in-source to an entry that is neither a regular file nor a directory, are rejected as preflight / validation errors under this section; installation does not commit the workflow, and no section 3.2 non-regular-`package.json` warning is emitted for that rejected source symlink.

Symlink targets must resolve to existing paths within the install source root, and to entries that are either regular files or directories. loopx must reject the install with a preflight / validation error when a symlink that is part of a selected workflow is broken, forms a cycle, resolves outside the source root, or resolves in-source to an entry that is neither a regular file nor a directory (FIFO, socket, block device, character device, or other non-regular non-directory entry). This prevents installed workflows from depending on the temporary clone / extraction directory, from copying arbitrary files outside the source, or from materializing non-file non-directory entries that have no general copy semantics.
