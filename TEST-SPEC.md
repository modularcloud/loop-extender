...
- **T-INST-60r**: With `-w`, the selected workflow's own unreadable `package.json` emits a single non-fatal warning and the install still succeeds. Same structure as T-INST-60q, but `ralph/package.json` is set to mode 000 (unreadable) instead of containing invalid JSON. `loopx install -w ralph <source>` → exit code 0. Assert: (a) stderr contains exactly one `package.json` read-failure warning for `ralph`, (b) `.loopx/ralph/` is installed and contains `index.sh` and the unreadable `package.json`, and (c) `.loopx/other/` does not exist. This is the unreadable-file counterpart to T-INST-60q (invalid JSON) — T-INST-74 covers the generic path; this test covers the selective `-w` path. **Conditional on `process.getuid() !== 0`.** *(Spec 10.8, 10.6, 3.2)*
...
- **T-INST-74**: Workflow `package.json` unreadable → warning, install proceeds (no version validation). **Conditional on `process.getuid() !== 0`.** *(Spec 10.6, 3.2)*
...
- **T-INST-79**: Staging failure leaves `.loopx/` unchanged. Multi-workflow source where `ralph/` is valid (contains `index.sh`) and `broken/` contains a script file (`index.sh`) plus a non-script file with permissions set to `000` (unreadable). `loopx install <source>` → staging fails when trying to copy the unreadable file in `broken/`. Assert: exit code 1, `.loopx/` is unchanged (neither `ralph/` nor `broken/` was written), and the error message identifies the failing workflow and the underlying cause. **Conditional on `process.getuid() !== 0`.** *(Spec 10.7)*
...
- **T-INST-80b**: `-y` replacement preserves the existing workflow when staging fails. Multi-workflow source where `ralph/` is valid (has `index.sh`) and `other/` contains a script file (`index.sh`) plus a non-script file with permissions set to `000` (unreadable). `.loopx/ralph/` already exists as a workflow with known content. `loopx install -y <source>` → staging fails when trying to copy the unreadable file in `other/`. Assert: exit code 1, the existing `.loopx/ralph/` is still intact with its original content (not removed or modified — the commit phase never ran). **Conditional on `process.getuid() !== 0`.** *(Spec 10.7)*
...
| 2.2 | Loop (state machine, goto) | T-LOOP-01–05, T-LOOP-15a, T-LOOP-16–19, T-LOOP-19a–19b, T-LOOP-30–43, T-LOOP-30a, T-LOOP-31a–31b, T-LOOP-32a, T-EXEC-16b, T-MOD-05a, T-MOD-21a, T-CLI-111a, T-API-08t2–08t8, T-API-14j2–14j8, T-API-35e, T-API-48a, T-VER-27, T-VER-27a–27c |
...
| 3.2 | Local Version Pinning & Delegation | T-DEL-01–27, T-DEL-14a, T-VER-01–27, T-VER-04a, T-VER-07a–07f, T-VER-10a–10b, T-VER-11a, T-VER-11b, T-VER-12a–12b, T-VER-13a, T-VER-13b, T-VER-18a, T-VER-19a–19c, T-VER-23a, T-VER-24a, T-VER-24b, T-VER-24c, T-VER-25a, T-VER-26a–26c, T-VER-27a–27c, T-CLI-119, T-CLI-119c, T-CLI-119e–119f, T-CLI-119h, T-CLI-119i–119k, T-MOD-03b, T-API-08aa, T-API-08b, T-API-08g, T-API-08o, T-API-08q, T-API-08s–08v, T-API-08u2, T-API-08u3, T-API-08w–08y, T-API-08z, T-API-08z2–08z4, T-API-08t2–08t8, T-API-14b2–14b4, T-API-14i–14l, T-API-14j2–14j8, T-API-14k2, T-API-14k3, T-API-14m–14o, T-API-14p, T-API-14p2–14p4, T-API-14q, T-API-20p, T-API-20q, T-API-20r, T-API-20s, T-INST-60q–60s, T-INST-72a, T-INST-73a |
...
| 8.3 | Env Injection Precedence | T-ENV-20–24, T-ENV-20a, T-ENV-21a, T-ENV-21b, T-ENV-21c, T-ENV-24a, T-ENV-24b, T-EXEC-03a, T-EXEC-04–04c, T-DISC-39a |
| 9.1 | run() | T-API-01–09c, T-API-08a–08e, T-API-08g–08m, T-API-08o–08v, T-API-08u2, T-API-08u3, T-API-08w–08y, T-API-08z, T-API-08z2–08z4, T-API-08aa, T-API-08t2–08t8, T-API-10–10c, T-API-20h–20i, T-API-20j–20m, T-API-20p, T-API-20r, T-API-30–37, T-API-35a–35e, T-API-36a, T-PARSE-20a, T-TYPE-04, T-TYPE-06–07 |
| 9.2 | runPromise() | T-API-08f, T-API-08n, T-API-08p2, T-API-11–14l, T-API-14b2–14b4, T-API-14j2–14j8, T-API-14k2, T-API-14k3, T-API-14m–14o, T-API-14p, T-API-14p2–14p4, T-API-14q, T-API-14a–14a3, T-API-20n, T-API-20o, T-API-20q, T-API-20s, T-API-25–25b, T-API-38–48, T-API-47a–47b, T-API-48a, T-API-44a–44b, T-PARSE-20a, T-TYPE-05–07 |
...
| 10.2 | Source Type Details | T-INST-56b, T-INST-57c, T-INST-81–89, T-INST-83a, T-INST-85a–85b, T-INST-86a |
| 10.3 | Workflow Classification | T-INST-50–56, T-INST-50a–50b, T-INST-52a, T-INST-52c, T-INST-53a–53c, T-INST-54a–54c, T-INST-55a–55e, T-INST-56a–56d, T-INST-57b, T-INST-60o, T-INST-64, T-INST-64b, T-INST-80g, T-INST-83a, T-INST-85b |
| 10.4 | Install-time Validation | T-INST-52a–52c, T-INST-61–64, T-INST-62a, T-INST-63a–63d, T-INST-64a–64d, T-INST-72a, T-INST-73a, T-INST-80h–80j, T-INST-60a, T-INST-60e, T-INST-60g, T-INST-60h, T-INST-60n |
...
| 10.6 | Version Checking on Install | T-INST-72–76, T-INST-72a, T-INST-73a, T-INST-97b, T-VER-12–13, T-VER-12a–12b, T-VER-13a, T-VER-13b, T-VER-15, T-VER-17, T-VER-22, T-VER-23a, T-INST-80d–80f, T-INST-54b, T-INST-60b, T-INST-60f, T-INST-60i, T-INST-60j, T-INST-60q–60s, T-DEL-25 |
