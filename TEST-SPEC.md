...
- **T-CLI-40a**: `loopx run -h` does not advertise legacy flat-script invocation syntax. Assert that stdout does **not** contain patterns reflecting the removed pre-workflow invocation model: (a) no usage line showing `loopx run [options] <script>` or `loopx run <script>` (the old flat-script form — the correct form uses `<workflow>[:<script>]`), and (b) no wording that refers to "script" as the primary positional argument without workflow context (e.g., bare "script-name" as a top-level concept). This is the run-help counterpart to T-CLI-02's negative assertions for top-level help, applied to the `run` subcommand where ADR-0003 changed the grammar most directly. *(Spec 4.2, 11.2)*
...
- **T-MOD-21a**: End-to-end bash cross-workflow goto via `$LOOPX_BIN output`. Create two workflows: `.loopx/alpha/index.sh` (runs `$LOOPX_BIN output --goto "beta:index"`) and `.loopx/beta/index.sh` (runs `$LOOPX_BIN output --result "arrived" --stop`). Run `loopx run alpha`. Assert: (a) execution transitions from `alpha:index` to `beta:index`, and (b) the final output contains `result: "arrived"` with `stop: true`. This is a composition test spanning sections 3.4 (LOOPX_BIN), 4.3 (loopx output subcommand), and 2.2 (cross-workflow goto). *(Spec 3.4, 4.3, 2.2)*
...
| 10.4 | Install-time Validation | T-INST-52a, T-INST-61–64, T-INST-62a, T-INST-63a–63c, T-INST-64a–64c, T-INST-80h–80j, T-INST-60a, T-INST-60e, T-INST-60g, T-INST-60h |
...
| 10.8 | Selective Workflow Installation | T-INST-57–60, T-INST-57a, T-INST-59a, T-INST-60a–60m, T-INST-64c |
