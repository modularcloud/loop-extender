...
- **T-INST-80c**: Commit-phase failure reports which workflows were and were not committed. Multi-workflow source with three workflows: `alpha/`, `beta/`, `gamma/` (each with `index.sh`). Set `LOOPX_TEST_INSTALL_FAULT=commit-fail-after:1` in the process environment to simulate a rename failure after committing one workflow (see section 1.4, install fault injection seam). Assert: (a) exit code 1, (b) error output identifies which workflow(s) were successfully committed and which were not, and (c) exactly one workflow directory exists in `.loopx/`. *(Spec 10.7)*
