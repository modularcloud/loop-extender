...
- `apps/tests/tests/helpers/fake-npm.ts` (`withFakeNpm`) — Bash shim recording every `npm install` invocation to a JSON-line log via a `trap __finalize EXIT` (so signal-induced termination still produces an entry; SIGKILL bypasses EXIT trap → empty log even if the shim ran, which 116c/116f rely on). For `recordGitignoreAtStart`, the shim reads `.gitignore` content via python3 (Bash command-substitution `$(cat)` strips trailing newlines). For `spawnFailure: true`, the helper builds a clean PATH dir of symlinks to every binary in original PATH except `npm` — necessary because libc's `execvp()` falls through to subsequent PATH entries on ENOENT, defeating dangling-symlink approaches. When `trapSignals` is set, the shim runs `while true; do sleep 1; done` instead of `sleep $SLEEP_FOR` so bash genuinely survives the SPEC §7.3 5-second grace window (a single `sleep` would die from the process-group signal even with `trap '' TERM`). For `spawnGrandchild`, the long-lived child is wrapped in `( trap - INT QUIT; exec sleep 3600 )` because non-interactive bash auto-sets SIGINT/SIGQUIT to SIG_IGN on `&`-async children, which would mask process-group SIGINT forwarding. Tests using `withFakeNpm` must be authored against `--no-install`-default behavior — SPEC §10.10 changed install semantics so legacy tests that previously asserted "no node_modules after install" now must add `--no-install` to preserve their original intent.
