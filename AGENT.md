...
- `apps/tests/tests/helpers/fake-npm.ts` (`withFakeNpm`) — Bash shim recording every `npm install` invocation to a JSON-line log via a `trap __finalize EXIT` (so signal-induced termination still produces an entry). For `recordGitignoreAtStart`, the shim reads `.gitignore` content via python3 (Bash command-substitution `$(cat)` strips trailing newlines). For `spawnFailure: true`, the helper builds a clean PATH dir of symlinks to every binary in original PATH except `npm` — necessary because libc's `execvp()` falls through to subsequent PATH entries on ENOENT, defeating dangling-symlink approaches. Tests using `withFakeNpm` must be authored against `--no-install`-default behavior — SPEC §10.10 changed install semantics so legacy tests that previously asserted "no node_modules after install" now must add `--no-install` to preserve their original intent.
