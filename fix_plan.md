...
**Status: ADR-0004 §6.1 (project-root cwd + LOOPX_WORKFLOW_DIR injection), §7.2 (first-trigger-wins surfaced-outcome under racing terminals — CLI signal-handler fix), §7.4 (LOOPX_TMPDIR creation/injection/cleanup + parent-snapshot timing across all four injection tiers + run-setup non-reaping + renamed-away ENOENT silence + symlink-replacement cleanup-rule-2 surface-parity + regular-file-replacement cleanup-rule-3 surface-parity + mismatched-directory cleanup-rule-5 surface-parity + nested-symlink-recursive-walk cleanup-rule-4 surface-parity + FIFO/socket/hard-link cleanup-rule-3 generic-dispatch coverage + lstat-fail / symlink-unlink-fail / recursive-remove-fail cleanup-failure surface-parity + cleanup-warning-does-not-mask-script-error surface-parity + cleanup-fault-warning-does-not-mask-script-error surface-parity + cleanup-warning-does-not-mask signal/abort terminal surface-parity + cleanup idempotence under racing SIGINT→SIGTERM at the cleanup-start window for rule-3 and rule-5 branches), §9.5 (RunOptions.env tier-2 env merging), §9.3 (abort-after-final-yield carve-out), §9.2 (LOOPX_TMPDIR async creation under runPromise()), AND §9.2 (process.env eager snapshot under runPromise()) are now IMPLEMENTED. Recent (this iteration): implemented the TEST-SPEC §1.4 `LOOPX_TEST_TERMINAL_TRIGGER_PAUSE` seam at the `cleanup-start` window value in `packages/loop-extender/src/tmpdir.ts` (writes a parent-observable `{"window":"cleanup-start"}` JSON marker via `openSync`/`writeSync`/`fsyncSync`/`closeSync` to the path named by `LOOPX_TEST_TERMINAL_TRIGGER_PAUSE_MARKER`, then sleeps 3s synchronously via `Atomics.wait` on a SharedArrayBuffer, gated on `NODE_ENV=test` and only when the env-var equals `cleanup-start`, fires after the `state.attempted` idempotence check so a second cleanup call on the same state is still a no-op); fixed the SPEC §7.2 first-trigger-wins surfaced-outcome contract in `packages/loop-extender/src/bin.ts` (signal handler now only sets `receivedSignal` if currently null, so a SIGTERM-during-cleanup-of-prior-SIGINT no longer overwrites the first signal observation and the surfaced exit code stays anchored at 130 not 143); authored T-TMP-38 (rule-3 regular-file-replacement) and T-TMP-39 (rule-5 mismatched-directory-replacement via rename-aside) covering CLI cleanup idempotence under racing SIGINT→SIGTERM. tmpdir.test.ts: 503/503 (was 499, +4 = 2 new IDs × 2 runtimes). Adjacent suites: programmatic-api + env-vars + wfdir (494/494), signals (9/9), loop-state + execution + cli-basics (449/449), unit + harness (158/158), zero regressions.**
