...
- **Final-Yield-vs-Settlement carve-out** — T-TMP-23, T-TMP-24, T-TMP-24a..24g (abort-after-final-yield × settlement matrix), T-TMP-24b (external SIGKILL leaks). Note: requires implementation work — after `yield output; return;` the generator currently returns cleanly without checking the abort signal; SPEC §9.3 line 922 requires that an abort in the post-final-yield/pre-settlement window produces an abort error on the next interaction (`.next()` / `.return()` / `.throw()`).
