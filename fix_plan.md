...
**Status: ADR-0004 §6.1 (project-root cwd + LOOPX_WORKFLOW_DIR injection), §7.4 (LOOPX_TMPDIR creation/injection/cleanup + parent-snapshot timing across all four injection tiers + run-setup non-reaping + renamed-away ENOENT silence + symlink-replacement cleanup-rule-2 surface-parity + regular-file-replacement cleanup-rule-3 surface-parity + mismatched-directory cleanup-rule-5 surface-parity + nested-symlink-recursive-walk cleanup-rule-4 surface-parity + FIFO/socket/hard-link cleanup-rule-3 generic-dispatch coverage + lstat-fail / symlink-unlink-fail / recursive-remove-fail cleanup-failure surface-parity + cleanup-warning-does-not-mask-script-error surface-parity), §9.5 (RunOptions.env tier-2 env merging), §9.3 (abort-after-final-yield carve-out), §9.2 (LOOPX_TMPDIR async creation under runPromise()), AND §9.2 (process.env eager snapshot under runPromise()) are now IMPLEMENTED. Recent (this iteration): authored T-TMP-35c/35d/35e covering SPEC §7.4 "cleanup warnings do not affect the CLI exit code, the generator outcome, or the promise rejection reason" on the script-error terminal × both warning-emitting cleanup-safety branches (regular-file replacement → rule 3; mismatched-directory replacement → rule 5) across all three execution surfaces. CLI test additionally pins observably-disjoint stderr lines for the script-failure marker vs. cleanup-warning marker (since both paths produce CLI exit 1, the marker line is the load-bearing distinguishing assertion). API tests pin the rejection / throw error message shape ("exited with code") with absence of "cleanup" classifier. The implementation already conformed to spec — `runLoop` (loop.ts:210-216) wraps the iteration body in `try/finally` so cleanupTmpdir runs before the script-failure throw propagates to consumer surfaces — so this iteration is purely test-authoring. 12 new tests (3 IDs × 2 variants × 2 runtimes). tmpdir.test.ts: 465/465 (was 453, +12). Adjacent suites: programmatic-api + env-vars + wfdir (494/494, zero regressions).**
