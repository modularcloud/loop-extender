...
**Status: ADR-0004 §6.1 (project-root cwd + LOOPX_WORKFLOW_DIR injection), §7.4 (LOOPX_TMPDIR creation/injection/cleanup + parent-snapshot timing across all four injection tiers + run-setup non-reaping + renamed-away ENOENT silence + symlink-replacement cleanup-rule-2 surface-parity + regular-file-replacement cleanup-rule-3 surface-parity + mismatched-directory cleanup-rule-5 surface-parity + nested-symlink-recursive-walk cleanup-rule-4 surface-parity + FIFO/socket/hard-link cleanup-rule-3 generic-dispatch coverage + lstat-fail / symlink-unlink-fail / recursive-remove-fail cleanup-failure surface-parity + cleanup-warning-does-not-mask-script-error surface-parity + cleanup-fault-warning-does-not-mask-script-error surface-parity), §9.5 (RunOptions.env tier-2 env merging), §9.3 (abort-after-final-yield carve-out), §9.2 (LOOPX_TMPDIR async creation under runPromise()), AND §9.2 (process.env eager snapshot under runPromise()) are now IMPLEMENTED. Recent (this iteration): authored T-TMP-42a covering SPEC §7.4 "cleanup warnings do not affect the CLI exit code, the generator outcome, or the promise rejection reason" on the SCRIPT-ERROR terminal × all three cleanup-fault seams (lstat-fail / symlink-unlink-fail / recursive-remove-fail) across all three execution surfaces (CLI / runPromise / run). CLI tests additionally pin observably-disjoint stderr lines for the script-failure marker vs. cleanup-warning marker (since both paths produce CLI exit 1). API tests pin the rejection / throw error message shape ("exited with code") with absence of "cleanup" classifier. The implementation already conformed to spec — `runLoop` (loop.ts:210-216) wraps the iteration body in `try/finally` so cleanupTmpdir runs before the script-failure throw propagates — so this iteration is purely test-authoring. 18 new tests (3 fault seams × 3 surfaces × 2 runtimes). tmpdir.test.ts: 483/483 (was 465, +18). Adjacent suites: programmatic-api + env-vars + wfdir (494/494, zero regressions).**
...
- **Cleanup-Safety dispatch matrix** — T-TMP-35f/35g/35h (cleanup-warning-does-not-mask-signal / abort across surfaces — require harness coordination for "wait for ready, send signal/abort, capture surfaced outcome" similar to T-API-10d-promise's pattern), T-TMP-38/39/38a/38a2/38b/38b-run/38b2/38b2-run/38c/38c2/38d/38d2/38d3/38d4/38e/38e-run/38f (cleanup idempotence and at-most-one-warning under racing terminals — require a new `LOOPX_TEST_TERMINAL_TRIGGER_PAUSE` seam not yet implemented), T-TMP-42b/42c (cleanup-failure × abort/signal terminal masking — require harness coordination for synchronized abort/signal-during-cleanup pattern). T-TMP-34/34a/34b (symlink-replacement cleanup-rule-2), T-TMP-35/35a/35b (regular-file-replacement cleanup-rule-3), T-TMP-36/36a/36b (mismatched-directory cleanup-rule-5), T-TMP-37/37d/37e (nested-symlink recursive-walk cleanup-rule-4), T-TMP-37a/37b/37c (FIFO/socket/hard-link rule-3 generic-dispatch coverage), T-TMP-35c/35d/35e (cleanup-warning-does-not-mask-script-error surface-parity), T-TMP-40/40a/40b (lstat-fail seam), T-TMP-41/41a/41b (symlink-unlink-fail seam), T-TMP-42/42d/42e (recursive-remove-fail seam), T-TMP-42a (cleanup-fault × script-error terminal masking) all RESOLVED in prior iterations.
