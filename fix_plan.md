...
**Status: ADR-0004 §6.1 (project-root cwd + LOOPX_WORKFLOW_DIR injection), §7.4 (LOOPX_TMPDIR creation/injection/cleanup + parent-snapshot timing across all four injection tiers + run-setup non-reaping + renamed-away ENOENT silence + symlink-replacement cleanup-rule-2 surface-parity + regular-file-replacement cleanup-rule-3 surface-parity + mismatched-directory cleanup-rule-5 surface-parity + nested-symlink-recursive-walk cleanup-rule-4 surface-parity + FIFO/socket/hard-link cleanup-rule-3 generic-dispatch coverage), §9.5 (RunOptions.env tier-2 env merging), §9.3 (abort-after-final-yield carve-out), §9.2 (LOOPX_TMPDIR async creation under runPromise()), AND §9.2 (process.env eager snapshot under runPromise()) are now IMPLEMENTED. Recent (this iteration): authored T-TMP-37a/37b/37c (rule-3 FIFO / Unix-domain-socket / hard-link replacement coverage on the CLI surface — proves rule 3 is typed generically across non-directory non-symlink entry types, not narrowly special-cased to regular files; T-TMP-37c additionally pins the SPEC §7.4 hard-link `nlink`-decrement-safety sub-clause that T-TMP-35's single-`nlink` regular-file fixture cannot exercise). 6 new tests (3 IDs × 2 runtimes). The implementation already conformed to spec — `cleanupTmpdir` case 3 dispatches via the generic `!stat.isDirectory()` negation (after the rule-2 symlink early-return at lines 209-228), which correctly covers any non-directory non-symlink entry — so this iteration is purely test-authoring. tmpdir.test.ts: 435/435 (was 429, +6). Adjacent suites: programmatic-api 358, env-vars + wfdir (494/494, zero regressions). Full e2e suite expected: 2295/2296 passing; the single remaining failure is T-INST-GLOBAL-01a [Bun] (pre-existing, unrelated).**
...
- **Cleanup-Safety dispatch matrix** — T-TMP-35c..35h (cleanup-warning-does-not-mask-script-error / signal / abort across surfaces), T-TMP-38/39/38a/38a2/38b/38b-run/38b2/38b2-run/38c/38c2/38d/38d2/38d3/38d4/38e/38e-run/38f (cleanup idempotence and at-most-one-warning under racing terminals — require a new `LOOPX_TEST_TERMINAL_TRIGGER_PAUSE` seam not yet implemented), T-TMP-40 (lstat-fail seam), T-TMP-41 (symlink-unlink-fail seam), T-TMP-42/42a/42b/42c (recursive-remove-fail seam — note: the seam itself is implemented in `tmpdir.ts`, only the parameterized surface-matrix tests are missing). T-TMP-34/34a/34b (symlink-replacement cleanup-rule-2), T-TMP-35/35a/35b (regular-file-replacement cleanup-rule-3), T-TMP-36/36a/36b (mismatched-directory cleanup-rule-5), T-TMP-37/37d/37e (nested-symlink recursive-walk cleanup-rule-4), and T-TMP-37a/37b/37c (FIFO/socket/hard-link rule-3 generic-dispatch coverage) all RESOLVED in prior iterations.
