...
**Status: NOT spec-conformant. T-WFDIR-01..14 + T-TMP foundational subset + T-TMP-12 (26 sub-cases × 2 surfaces) implemented; remaining P1 gaps in ADR-0004 coverage remain.**
...
- **CLI counterpart** — T-TMP-12-cli (parameterized 10 CLI-observable pre-iteration failure modes), T-TMP-12-cli-usage (parser-level + run-help short-circuit, 6 sub-cases).
- **Tmpdir-creation failure paths** — T-TMP-12a/12b/12c (mkdtemp-fails on unwritable parent, across CLI / `run` / `runPromise`), T-TMP-12d/12d2/12e/12e2/12e3 (LOOPX_TEST_TMPDIR_FAULT seam coverage), T-TMP-12f..12f5/12g/12h (version-check-vs-tmpdir-creation ordering).
