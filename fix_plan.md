...
**Status: ADR-0004 §6.1 (project-root cwd + LOOPX_WORKFLOW_DIR injection), §7.4 (LOOPX_TMPDIR creation/injection/cleanup + parent-snapshot timing across all four injection tiers + run-setup non-reaping + renamed-away ENOENT silence + symlink-replacement cleanup-rule-2 surface-parity + regular-file-replacement cleanup-rule-3 surface-parity + mismatched-directory cleanup-rule-5 surface-parity + nested-symlink-recursive-walk cleanup-rule-4 surface-parity + FIFO/socket/hard-link cleanup-rule-3 generic-dispatch coverage + lstat-fail / symlink-unlink-fail / recursive-remove-fail cleanup-failure surface-parity), §9.5 (RunOptions.env tier-2 env merging), §9.3 (abort-after-final-yield carve-out), §9.2 (LOOPX_TMPDIR async creation under runPromise()), AND §9.2 (process.env eager snapshot under runPromise()) are now IMPLEMENTED. Recent (this iteration): authored T-TMP-40/40a/40b (lstat-fail), T-TMP-41/41a/41b (symlink-unlink-fail), T-TMP-42/42d/42e (recursive-remove-fail) covering the SPEC §7.4 final paragraph "single stderr warning, no further changes" branches across all three execution surfaces (CLI / runPromise / run). The implementation already conformed to spec — the LOOPX_TEST_CLEANUP_FAULT seam at three failure-injection points in `cleanupTmpdir` (top-of-routine for lstat-fail, rule-2 unlink for symlink-unlink-fail, rule-4 rmSync for recursive-remove-fail) routes through `emitCleanupWarning` which deduplicates via `CleanupState.warned` — so this iteration is purely test-authoring. 18 new tests (9 IDs × 2 runtimes). tmpdir.test.ts: 453/453 (was 435, +18). Adjacent suites: programmatic-api + env-vars + wfdir (494/494, zero regressions). Full e2e suite expected: 2313/2314 passing; the single remaining failure is T-INST-GLOBAL-01a [Bun] (pre-existing, unrelated).**
