...
- **§8 — Environment Variables.** Add `LOOPX_TMPDIR` to the injected-variables table. Document that `RunOptions.env` merges after env files (global + local) and before loopx-injected protocol variables. Rewrite the §8.3 precedence list (highest wins) to the five tiers:
