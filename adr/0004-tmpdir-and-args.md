...
- **Created:** once per `loopx run` invocation, after the starting workflow's version check and immediately before the first iteration's child process is spawned. The pre-iteration sequence (extending SPEC §7.1) differs slightly between the CLI and the programmatic API, because the programmatic API exposes a `RunOptions` object whose fields must be validated before any runtime work begins.
  ...
  **Programmatic (`run()` / `runPromise()`) pre-iteration sequence:**
  1. Field-level validation of the call-site inputs. The `target` parameter must be a string — non-string values (`undefined`, `null`, numbers, booleans, objects, etc.) are rejected here, independent of what the value would otherwise resolve to. The `RunOptions` fields are then type-checked: `args` must be an array of strings or `undefined`; `maxIterations` must be a non-negative integer or `undefined`; `envFile` and `cwd` must be strings or `undefined`; `signal` must be an `AbortSignal` or `undefined`. For validation purposes, "an `AbortSignal`" means `instanceof AbortSignal` against the platform global constructor of the runtime that called `run()` / `runPromise()` (Node.js ≥ 20.6 and Bun ≥ 1.0 both provide a global `AbortSignal`). Duck-typed or cross-realm objects that expose an `aborted` property and `addEventListener` but are not `instanceof AbortSignal` are rejected at step 1. Filesystem-level and semantic checks — whether `envFile` exists, whether `cwd` contains a valid `.loopx/`, whether the `target` *string* matches the workflow/script name pattern or resolves to an existing target — are deferred to later steps and are **not** step-1 concerns. **Order within step 1:** the `target` parameter is validated before any `RunOptions` field; the internal order among `RunOptions` field type-checks is implementation-defined. The externally-guaranteed contract is only that a non-string `target` surfaces in preference to any invalid `RunOptions` field, and that any step-1 field-validation error surfaces in preference to all downstream steps (2 through 9 below).
  2. Already-aborted `AbortSignal` check. If step 1 succeeded and `RunOptions.signal` was provided (and is therefore a valid `AbortSignal` per step 1), loopx inspects its `.aborted` property. If `.aborted` is already `true`, the run is aborted here: the generator throws an abort error (for `run()`) or the promise rejects with an abort error (for `runPromise()`), and no further pre-iteration work runs. This step is reached only after step 1 has succeeded — a field-validation error from step 1 wins over an already-aborted signal. An already-aborted signal at step 2 wins over every later step (discovery, env loading, target resolution, `maxIterations: 0`, version check, tmpdir creation, first spawn).
  ...
  **Precedence among pre-spawn failures is the order shown above.** When multiple failure modes are present, the earliest-numbered step wins. Under the programmatic API, this means field validation (step 1 — non-string `target` and invalid `RunOptions` field types) runs **before** the already-aborted signal check, discovery, env loading, target syntax/existence resolution, and `maxIterations: 0`. A non-string `target` or an invalid `args` / `maxIterations` / `envFile` / `cwd` / `signal` therefore surfaces in preference to an already-aborted signal, a missing `.loopx/`, a missing env file, a missing workflow, a missing script, or the `maxIterations: 0` early-exit branch. An already-aborted signal (step 2) wins over discovery, env loading, target resolution, `maxIterations: 0`, the version check, tmpdir creation, and first spawn — but does **not** win over step 1. Env-file loading (step 4) runs before target syntax/existence resolution (step 5), so a missing or malformed env file surfaces in preference to a missing workflow or missing script. Under the CLI, argument parsing (step 1) runs before any of the runtime pre-iteration steps, so `-n` non-integer values, unknown flags, missing flag values, and duplicate flags surface in preference to project-level errors.

  Errors that occur before the tmpdir-creation step — CLI parser errors, programmatic field-validation failures, already-aborted-signal aborts, discovery errors, env-file loading failures, starting-target resolution failures, and `-n 0` / `maxIterations: 0` — do not create a `LOOPX_TMPDIR`, and no cleanup is needed for these cases. The version check is non-fatal and therefore does not itself suppress tmpdir creation.
...
- **Removed when the loop ends**, in all of the following cases:
  ...
  - SIGINT / SIGTERM: if a child process group is active, cleanup runs after the process group has exited (per the grace period in SPEC §7.3, including `SIGKILL` escalation to the process group if required) and before loopx itself exits with the signal's exit code. If no child process is active when the signal arrives — including (a) the window after `LOOPX_TMPDIR` has been created but before the first child has spawned, and (b) the window between a child exiting and the next child spawning (for example, between a script exit and the next `goto`-resolved child starting) — cleanup runs immediately, and loopx then exits with the signal's exit code. Pre-tmpdir-creation arrivals are covered by SPEC §7.3's "between iterations" rule and require no cleanup.
  - Programmatic `AbortSignal` abort: if a child process group is active, loopx first terminates it using the cancellation behavior defined in SPEC §9.1; cleanup runs only after the child process group has exited or been killed, and before the generator throws or the promise rejects. If the signal is aborted while no child process is active — including (a) while the generator is suspended between `next()` calls, and (b) during the window after `LOOPX_TMPDIR` has been created but before the first child has spawned — loopx's signal listener performs cleanup immediately when the abort event fires; the next generator interaction (or the in-flight first `next()` call, in the post-tmpdir / pre-spawn case) settles with the abort error as soon as cleanup completes, and the `runPromise()` promise (if outstanding) rejects with the abort error as soon as cleanup completes. The caller is not required to resume or explicitly close the generator to observe cleanup in this case.
...
- **`LOOPX_TMPDIR` path replacement between creation and cleanup:** if the `LOOPX_TMPDIR` path has been replaced between creation and cleanup (for example, a script removes the tmpdir and creates a symlink of the same name pointing elsewhere, removes the tmpdir and creates an unrelated directory at the original path, removes the tmpdir and creates a regular file at the original path, or renames the tmpdir and creates a symlink at the original path), cleanup must never traverse into or recursively delete any directory, symlink target, or other location that loopx did not originally create as this run's scratch space. The observable guarantee is: the only directory contents loopx may recursively remove are the contents of the directory whose device/inode identity matches the original tmpdir; any other entry at the tmpdir path is at most unlinked as a single non-traversing entry. Because loopx v1 is POSIX-only (SPEC §1 Platform support), the device/inode pair returned by `lstat` is always available, and loopx records it immediately after the `mkdtemp` that creates the tmpdir. At cleanup time, loopx performs `lstat` on the `LOOPX_TMPDIR` path and dispatches on the result:
  ...
  3. If the path is a non-directory, non-symlink entry (regular file, FIFO, socket, etc.), loopx unlinks that entry. This is safe because the `LOOPX_TMPDIR` path is unique to this run — loopx owns the path even if the entry occupying the path has been replaced — and `unlink` on a non-directory entry does not traverse into any other location on the filesystem.
  ...
  Implementation note: the invariant is guaranteed by the device/inode identity comparison — the recursive-removal branch (case 4) runs only when `lstat` confirms the path resolves to the same directory that `mkdtemp` returned. Symlink-safety applies equally to entries inside the tmpdir and to the tmpdir path itself: cleanup never walks into a symlink's target, and never recurses into a directory other than the one whose identity was recorded at creation, regardless of whether the replacement was introduced inside the tmpdir or in place of the tmpdir. Workflow state such as `.loopx/`, `node_modules/`, or any location reachable only through a symlink or a post-creation substitution at the tmpdir path is never touched by cleanup.
...
Pre-spawn validation failures that precede tmpdir creation — non-string `target`, invalid `RunOptions.args`, invalid `RunOptions.maxIterations`, invalid `RunOptions.envFile`, invalid `RunOptions.cwd`, invalid `RunOptions.signal`, already-aborted `AbortSignal`, discovery errors, env-file loading failures, starting-target resolution failures (invalid target syntax, missing workflow, missing script), and `-n 0` / `maxIterations: 0` — do **not** create a tmpdir, and do not reach the tmpdir-creation step. This is consistent with the "no tmpdir for pre-spawn failures" rule stated under Scope and lifecycle above.
...
- **7.1 (Basic Loop)** — Insert a new pre-iteration step between the current step 5 (starting workflow version check) and the current step 6 (first execution): **create and secure `LOOPX_TMPDIR`**. For the CLI, after successful run-subcommand parsing (section 4.1) — including the `-h` / `--help` short-circuit, which exits before any runtime work — the runtime pre-iteration order is (1) discovery, (2) env loading, (3) starting-target resolution, (4) `-n 0` early exit, (5) starting workflow version check, (6) `LOOPX_TMPDIR` creation, (7) first execution. Run-subcommand parser errors (unknown flags, missing flag values, duplicate flags, non-integer `-n`, etc.) surface before any runtime pre-iteration step runs and are out of scope for §7.1; they are governed by §4.1 and §4.2. The programmatic API's pre-iteration sequence additionally begins with a step-1 field-validation check (target parameter type, plus `RunOptions` field types for `args`, `maxIterations`, `envFile`, `cwd`, `signal`) that runs before discovery, followed by a step-2 already-aborted `AbortSignal` check that runs after field validation succeeds but before discovery; full ordering and precedence among pre-spawn failures are documented in §9.1 / §9.2. Add: the arg vector is captured at loop start from CLI positional args (or `RunOptions.args`) and is passed unchanged to every script executed during the run, including scripts reached via `goto` and the starting target on loop reset. Add: on normal loop completion — `stop: true` (current step 9) or `-n` / `maxIterations` reached (current step 10) — `LOOPX_TMPDIR` cleanup runs before loopx exits with code 0. For the programmatic API, the final iteration's output is yielded to the consumer before the generator settles; `LOOPX_TMPDIR` cleanup runs after that final yield and before the generator's terminal `{ done: true }` transition (and, for `runPromise()`, before the outstanding promise resolves), **except** when an `AbortSignal` abort intervenes in that window — abort continues to take precedence until the generator has settled terminally, per the abort-after-final-yield rule documented in §9.1 / §9.2. Subject to the manual-abandonment caveat documented under §9.1 / §9.2, consumers who drive `run()` via `for await (...)` or who use `runPromise()` observe cleanup as part of normal completion. The full `LOOPX_TMPDIR` lifecycle across normal completion, error exit, signals, and programmatic cancellation is defined in the new §8.4.
...
- Verify programmatic pre-spawn precedence: when a call combines multiple pre-spawn failures, the surfaced error reflects the earliest-numbered step in the programmatic pre-iteration sequence (field validation → already-aborted signal → discovery → env loading → target resolution → `maxIterations: 0` → version check → tmpdir → spawn). In particular:
