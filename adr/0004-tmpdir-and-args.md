...
- **Symlink at the `LOOPX_TMPDIR` path itself:** if the `LOOPX_TMPDIR` path has been replaced between creation and cleanup (for example, a script removes the tmpdir and creates a symlink of the same name pointing elsewhere, removes the tmpdir and creates an unrelated directory at the original path, or renames the tmpdir and creates a symlink at the original path), cleanup must not walk into or delete anything that loopx did not create as the run's scratch space. To make this precise, loopx records the originally-created tmpdir's filesystem identity — where available on the platform, the device/inode pair returned by `lstat` on the directory immediately after creation — and, at cleanup time, performs `lstat` on the `LOOPX_TMPDIR` path and dispatches on the result:
  ...
  3. If the path is a non-directory, non-symlink entry (regular file, FIFO, socket, etc.), loopx unlinks that entry.
  4. If the path is a directory and its recorded identity matches the directory observed now (same device/inode), loopx recursively removes it, applying the "symlinks inside `LOOPX_TMPDIR`" rule above: symlink entries found during the walk are unlinked but never traversed.
  5. If the path is a directory but its identity does not match the recorded identity (e.g., the original tmpdir was removed and an unrelated directory was created in its place), loopx does **not** recursively remove it. Loopx prints a single cleanup warning to stderr identifying the path and the mismatch, and proceeds. The unrelated directory is left intact.
  6. If identity information is unavailable on the platform, loopx falls back to the directory case only when `lstat` confirms the path is a directory and is not a symlink; otherwise it applies the appropriate unlink-only branch (case 2 or 3). In the fallback directory case, loopx proceeds with recursive removal under the same symlink-safety rules — the platform fallback does not weaken the symlink-traversal ban.

  This preserves the invariant that cleanup affects only the run's scratch space and cannot collateral-delete workflow state such as `.loopx/`, `node_modules/`, or any location reachable only through a symlink or through a post-creation substitution at the tmpdir path. Symlink-safety applies equally to entries inside the tmpdir and to the tmpdir path itself: cleanup never walks into a symlink's target, regardless of whether the symlink was introduced inside the tmpdir or in place of the tmpdir.
...
- **9.3 (Error Behavior)** — Add: when a `LOOPX_TMPDIR` has been created for the run, cleanup of the tmpdir runs before the generator throws or the promise rejects. The original error is preserved; a tmpdir-cleanup failure produces a single warning on stderr but does not replace or mask the original error's identity or message. When no tmpdir was created — because of a pre-spawn failure, `-n 0` / `maxIterations: 0`, or an abort before the first spawn — no cleanup is needed. The underlying outcome (normal completion, error, or abort) proceeds unchanged: `-n 0` / `maxIterations: 0` is **not** an error when the options are valid, and is observed as normal completion with no outputs yielded (generator completes with `{ done: true }` and no prior yields; `runPromise()` resolves with `[]`).
