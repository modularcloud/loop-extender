...
- **Created:** once per `loopx run` invocation, after the starting workflow's version check and immediately before the first iteration's child process is spawned. The pre-iteration sequence for `loopx run` (extending SPEC §7.1) is:
  1. Discovery and name/collision validation.
  2. Environment variable loading (global + `-e` local).
  3. Starting-target resolution.
  4. `-n 0` / `maxIterations: 0` early exit (if requested).
  5. Starting workflow version check (non-fatal per SPEC §3.2 — a mismatch emits a stderr warning and continues).
  6. **Create and secure `LOOPX_TMPDIR`** (this ADR).
  7. Spawn the first child process.

  Errors that occur before step 6 — discovery errors, starting-target resolution failures, env-file loading failures, and `-n 0` / `maxIterations: 0` — do not create a `LOOPX_TMPDIR`, and no cleanup is needed for these cases. The version check at step 5 is non-fatal and therefore does not itself suppress creation at step 6.
...
- **7.1 (Basic Loop)** — Insert a new pre-iteration step between the current step 5 (starting workflow version check) and the current step 6 (first execution): **create and secure `LOOPX_TMPDIR`**. The revised pre-iteration order is (1) discovery, (2) env loading, (3) starting-target resolution, (4) `-n 0` / `maxIterations: 0` early exit, (5) starting workflow version check, (6) `LOOPX_TMPDIR` creation, (7) first execution. Add: the arg vector is captured at loop start from CLI positional args (or `RunOptions.args`) and is passed unchanged to every script executed during the run, including scripts reached via `goto` and the starting target on loop reset. Add: on normal loop completion — `stop: true` (current step 9) or `-n` / `maxIterations` reached (current step 10) — `LOOPX_TMPDIR` cleanup runs before loopx exits with code 0. For the programmatic API, the final iteration's output is yielded to the consumer before the generator settles; `LOOPX_TMPDIR` cleanup runs after that final yield and before the generator's terminal `{ done: true }` transition (and, for `runPromise()`, before the outstanding promise resolves). Subject to the manual-abandonment caveat documented under §9.1 / §9.2, consumers who drive `run()` via `for await (...)` or who use `runPromise()` observe cleanup as part of normal completion. The full `LOOPX_TMPDIR` lifecycle across normal completion, error exit, signals, and programmatic cancellation is defined in the new §8.4.
