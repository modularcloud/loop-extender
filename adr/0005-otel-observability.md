...
| Key | Default | Meaning |
|-----|---------|---------|
| `LOOPX_OTEL_ENABLED` | unset (disabled) | `"true"` or `"1"` enable the SDK. `"false"`, `"0"`, and absence disable it. Any other value also disables and is **not** treated as an "invalid `LOOPX_OTEL_*` value": no stderr warning fires, even when other OTel warnings would. This is a deliberate special case so that `loopx otel disable` (which writes `LOOPX_OTEL_ENABLED=false`) is silent and so that user typos default-safely to "off" without noise. |
| `LOOPX_OTEL_CAPTURE_RESULT` | `"none"` | `"none"`: record `result` byte length always, and a hex SHA-256 only when the payload is non-empty (matching the `loopx.output.result.sha256` span-attribute rule in §4). `"truncated"`: same as `"none"` plus a preview attribute holding the first N bytes. `"full"`: record entire `result` string as a span attribute. Any other value falls back to `"none"` with a stderr warning. |
| `LOOPX_OTEL_CAPTURE_RESULT_TRUNCATE_BYTES` | `"4096"` | Byte cap for `"truncated"` mode. Must parse as a non-negative integer; invalid value falls back to default with a stderr warning. A value of `0` is valid and **omits** the `loopx.output.result.preview` attribute (no zero-length string is recorded), matching the omit-when-empty rule used for `loopx.output.result.sha256` at `bytes == 0` (§4). |
| `LOOPX_OTEL_CAPTURE_STDIN` | `"none"` | Same three values applied to script stdin (the `result` piped from the previous iteration). Byte length is always recorded; a hex SHA-256 is recorded only when the payload is non-empty (matching the `process.stdin.sha256` span-attribute rule in §4). Same fallback rule on invalid value. |
| `LOOPX_OTEL_CAPTURE_STDIN_TRUNCATE_BYTES` | `"4096"` | Byte cap for stdin truncation. Same validation as the result variant; a value of `0` omits the `process.stdin.preview` attribute, matching the omit-when-empty rule for `process.stdin.sha256`. |
...
Any key matching `OTEL_*` (the SDK's standard env-var namespace) is also recognized, validated by name pattern only (`[A-Za-z_][A-Za-z0-9_]*`), and passed to the SDK and (per §9) to children when propagation is on. loopx does not validate semantic correctness of `OTEL_*` values — that is the SDK's job — **except `OTEL_TRACES_EXPORTER` and `OTEL_METRICS_EXPORTER`, which are normalized by loopx per §10** before SDK initialization, child env propagation (§9 tier 3), and `loopx otel show` / `loopx otel test` display. For all other `OTEL_*` keys, invalid values produce SDK-side warnings on stderr at startup.
...
**"Otel active" run state.** Two predicates govern telemetry behavior and must not be conflated:

1. **Effective enabled** — the resolved value of `LOOPX_OTEL_ENABLED` for this run, computed from `options.otel.enabled` (when supplied) layered over the otel config file's value. This is determined before SDK initialization is attempted.
2. **Otel active** — effective enabled is `true` **and** SDK initialization has succeeded for this run (per §10's deferred init step and per-run init-failure rule).

Behaviors gated on **otel active** (i.e., suppressed when effective enabled is `true` but SDK init fails):

- `loopx.*` span emission and metric emission (§4, §7).
- Per-run `forceFlush()` and (CLI) SDK `shutdown()` at terminal (§10).
- `TRACEPARENT` / `TRACESTATE` injection at the loopx-injected protocol-variable tier (§5).
- Tier-3 OTel-config propagation into child environments, including the injected `LOOPX_OTEL_ENABLED=true` (§9).
- Reservation of `TRACEPARENT` / `TRACESTATE` against being shadowed by lower env tiers (§12).

When effective enabled is `true` but SDK init fails, loopx records the single stderr warning specified in §10 and, for the rest of that run's lifetime, suppresses every otel-active behavior — no spans, no metrics, no `TRACEPARENT` injection, no tier-3 propagation, no `LOOPX_OTEL_ENABLED=true` reaching children. Children's environments are computed exactly as they would be on a default-off run; user-supplied lower-tier values (inherited `TRACEPARENT`, `OTEL_*`, `LOOPX_OTEL_ENABLED`, etc.) still flow through unchanged per the byte-identity guarantee below. Subsequent runs in the same process attempt SDK init again per §10.

Behaviors gated on **effective enabled** alone (and therefore *not* suppressed by SDK init failure for the run):

- The single stderr warning surfaced by `options.otel.enabled=true` against an unreadable otel config file (§1, "Failure modes" below) — this fires before SDK init is attempted.
- Warning gating itself for OTel config-file parse warnings (§1 "Failure modes," §10 "Warning policy"). A run whose SDK init later fails still has these warnings surface; the warnings are caused by reading the file at all, which is what telemetry being effectively enabled implies.

Throughout the rest of this ADR, "otel enabled" wording in normative statements should be read as one of these two predicates. Where ambiguity matters, sections use the explicit terms "otel active" or "effective enabled" instead.
...
- **Invalid `LOOPX_OTEL_*` value:** stderr warning; fallback to the default value. Excludes `LOOPX_OTEL_ENABLED`, which is permissive per the table above — any non-enabling value disables silently with no warning.
...
2. **Otel config file disk read and SDK initialization.** Both happen after the pre-iteration sequence completes successfully (discovery, env-file loading, target resolution, starting-workflow version check, tmpdir creation) and immediately before the first child spawn — for **all** invocation modes (CLI, `run()`, `runPromise()`). Pre-iteration failures **and `-n 0` / `maxIterations: 0` early exits** cause no config-file read, no SDK initialization, and no otel-related warning, preserving byte-identical pre-ADR behavior on those paths. For `runPromise()`, the inherited-env capture in (1) happens synchronously at the call site, but the disk read of the config file in (2) is deferred until pre-iteration succeeds asynchronously after return. This separation is what preserves both byte-identity on pre-iteration failures and existing `runPromise()` snapshot semantics.

**`-n 0` / `maxIterations: 0` is OTel-dormant.** Per SPEC §7.1 step 4, an `-n 0` (or `maxIterations: 0`) invocation exits successfully **before** workflow-level version checking (step 5) and tmpdir creation (step 6). Because SDK initialization is deferred until after tmpdir creation per (2) above, an `-n 0` early-exit run reads no otel config file, initializes no SDK, opens no `loopx.run` span, emits no metrics, injects no `TRACEPARENT` into any environment, and produces no OTel-related stderr warnings — even when `LOOPX_OTEL_ENABLED=true` in the otel config file or `options.otel.enabled=true` is supplied. Behavior is byte-identical to a default-off run.
...
- **`loopx otel set <name> <value>`** — mirrors `loopx env set`: same name pattern (`[A-Za-z_][A-Za-z0-9_]*`), same value rules (no `\n` / `\r`), same `KEY="<literal value>"` serialization. Accepts any name matching the pattern (no enforcement of `OTEL_*` / `LOOPX_OTEL_*` prefix, consistent with `loopx env set`); unrecognized names are silently retained but ignored at SDK-init time. **Values are not validated semantically at set time**, even for recognized `LOOPX_OTEL_*` keys with a fixed value vocabulary (e.g., `LOOPX_OTEL_CAPTURE_RESULT`, `LOOPX_OTEL_CAPTURE_RESULT_TRUNCATE_BYTES`). The supplied value is written verbatim with no stderr output and no exit-code change; the per-key invalid-value warning specified in §1's recognized-keys table fires at the runtime path (SDK init), not at `loopx otel set`. This preserves the "permissive at set, semantic at runtime" rule already established for unrecognized names. `loopx otel show` reflects the post-fallback default for the affected key when it next runs (matching the unsupported-exporter pattern in §1 / §2's `show` description).
- **`loopx otel remove <name>`** — mirrors `loopx env remove`: silent no-op if absent.
- **`loopx otel list`** — prints the **parsed config-file-only** key/value map, after `.env` parsing and duplicate-key resolution (last-occurrence-wins, per §1), one `KEY=VALUE` per line, sorted lexicographically, no output if empty. Mirrors `loopx env list` in format. Does **not** include inherited `process.env` `OTEL_*` values and does **not** apply redaction; this is the unredacted raw-file view, distinct from `show` (which displays the merged effective configuration with redactions applied). Malformed lines (no `=` separator) and lines whose key does not match `[A-Za-z_][A-Za-z0-9_]*` are skipped with one stderr warning per offending line, per the parse-level rule below. **Semantically invalid recognized `LOOPX_OTEL_*` values remain visible as their raw configured value** (`list` performs name-and-shape parsing only — value-vocabulary enforcement is a SDK-init-time concern). To see the post-fallback effective values, use `loopx otel show`.
- **`loopx otel show`** — human-readable summary of **effective** configuration: enabled state, effective endpoint, protocol, sampler, service name, capture-result mode, propagate-to-scripts mode, plus a one-line note for any key whose value is non-default. "Effective" here is the merge of the otel config file with inherited `process.env` `OTEL_*` (file values win on conflict, mirroring the parent-SDK config-resolution order in §1) — not just the raw file. Distinct from `list`, which shows only the parsed config-file-only key/value map without inherited `OTEL_*` and without applying value-vocabulary fallbacks. **The unsupported-exporter fallback (§10) applies to the displayed effective values:** when the resolved value of `OTEL_TRACES_EXPORTER` or `OTEL_METRICS_EXPORTER` is unsupported (anything other than `otlp` or `none`), `show` displays the post-fallback value `otlp` and emits the same single-stderr-warning-per-offending-key that `loopx run` would emit at SDK init. This makes `show`'s output reflect what `loopx run` will actually use, even though `show` does not initialize the SDK. **Credential-bearing values are redacted in `show`**: any key whose name (case-insensitive) contains any of `HEADERS`, `TOKEN`, `AUTH`, `PASSWORD`, `SECRET`, `CREDENTIAL`, or `BEARER` prints with its value replaced by `<redacted>` while the key name remains visible. The substring list is intentionally narrow and may be extended in future minor versions; adding new substrings is not a breaking change. `loopx otel list` is the unredacted view for users who need raw values. `loopx otel show` does not initialize the SDK and does not require `.loopx/`.
- **`loopx otel test`** — initializes the SDK from current effective config, emits one probe per enabled exporter, runs a 5-second `forceFlush()` + `shutdown()`, prints `OK <endpoint>` to stdout and exits 0 on success, prints the underlying SDK error to stderr and exits 1 on failure. When otel is disabled in config, prints ``disabled — run `loopx otel enable` first`` to stderr and exits 1; the disabled-state check **short-circuits all further validation and warnings** — invalid or unsupported `LOOPX_OTEL_*` / `OTEL_*` values present in the config file are not surfaced by `test` on this code path. Use `loopx otel show` (which validates and emits per-key warnings regardless of enabled state) for validation independent of enable state. The enable bit consulted by `loopx otel test` is the otel config file's `LOOPX_OTEL_ENABLED` only; shell-environment `LOOPX_OTEL_ENABLED` is not consulted, matching the parent-SDK enable rule in §1. `loopx otel test` is the **one documented exception** to exporter-failure isolation (§10): SDK / exporter errors are surfaced on stderr and reflected in the exit code, since the command exists to verify connectivity. "OK" means "the SDK exported without raising an error", not "the backend has indexed the probe".
...
**Duplicate-key behavior on rewrite.** When the file already contains the managed key in multiple positions (per the file format's "last occurrence wins" parse rule, mirrored from SPEC §8.1):

- `enable`, `disable`, and `set <name> <value>` collapse all occurrences of the managed key to a **single** occurrence. The retained occurrence lands at the position of the **first** existing occurrence and carries the new value; subsequent occurrences of the same key are removed. This makes the post-rewrite file consistent with the parsed value.
- `remove <name>` deletes **every** occurrence of the named key.
- All other lines — including comments, blanks, unrelated keys, and unrelated keys that happen to be duplicated — are preserved unchanged in their original positions.

**Config-file parse-level errors (readable file, invalid line shape).** Two classes of malformed line, both **not fatal** for config-management commands; both mirror the runtime rule in §1 and the global env file's behavior in SPEC §8.1. Each offending line emits one stderr warning per parse and is otherwise skipped:
...
Parse-level warnings are subject to the runtime gating in §1 and §10's warning policy: they fire only when telemetry is effectively enabled for the run, or when a `loopx otel *` config-management command is explicitly invoked. `enable` / `disable` / `set` / `remove` rewrites preserve the offending lines unchanged (they neither scrub nor rewrite them beyond the specific key being managed by the command) but do warn when they read the file before writing. Unreadable file (above) is fatal for config-management commands; readable-but-invalid is warn-and-ignore.

**Recognized-key value validation (semantic-level, not parse-level).** Recognized `LOOPX_OTEL_*` keys whose values do not satisfy the per-key validation in §1's recognized-keys table are **not parse-level errors**: parsing succeeds, the line is retained verbatim at the file level, and the value-fallback warning is deferred. Behavior is consistent across commands:

| Command | Behavior on a recognized key with a semantically invalid value |
|---|---|
| `loopx otel list` | Shows the offending key with its raw configured value. No fallback applied, no warning emitted. |
| `loopx otel set` | Writes the supplied value verbatim. No semantic validation, no warning, no exit-code change. |
| `loopx otel enable` / `disable` / `remove` | Preserve the offending line unchanged at rewrite time. No fallback, no warning attributable to the offending key. |
| `loopx otel show` | Displays the post-fallback effective value (the documented default for the key) and emits the per-key invalid-value warning at most once per offending key per process. |
| `loopx otel test` | When the run is effectively enabled, applies the same fallback + warning at SDK init. When the run is disabled, the disabled-state rule short-circuits all further validation and no warning fires; use `show` for validation independent of enabled state. |
| Runtime path (`loopx run` / `run()` / `runPromise()`) | Applies fallback + warning at SDK init when the run is otel active. |

Excludes `LOOPX_OTEL_ENABLED`, which is permissive per §1's table — any non-enabling value disables silently with no warning.
...
- **`loopx otel span <name> ... -- <command>`** — runs `<command>` as a child process. Two modes:
  ...
  **Span mode** — when otel is enabled (`LOOPX_OTEL_ENABLED=true` or `1` in the helper's env) **and** a valid W3C `TRACEPARENT` is present in the helper's environment. These are the **only** two gating predicates; "reachability" of an `OTEL_*` endpoint is not a gating predicate. The helper initializes its own SDK from whatever `OTEL_*` configuration is reachable in inherited env (including SDK defaults when no explicit endpoint is set), starts a span as a child of the `TRACEPARENT` context, **spawns** the wrapped command as a child process (does not `exec`), injects both `TRACEPARENT` and, when non-empty, `TRACESTATE` derived from the new span's context into the spawned command's environment (mirroring the parent loopx process's child-env injection in §5), awaits exit. **Helper exit code:** when the wrapped command exits normally, the helper exits with the wrapped command's exit code byte-for-byte. When the wrapped command terminates by signal (whether by a signal delivered to the shared process group or by an external sender, including SIGKILL), the helper exits with `128 + signal_number`, matching the loopx-wide shell convention used elsewhere in this ADR (§4 `process.exit_code` on `loopx.script.exec`, §7 `loopx.script.exit_code` counter, SPEC §7.3). **Span status:** when the wrapped command exits normally, span status is taken from `--status` if provided, otherwise derived from exit code (0 → OK, non-zero → ERROR). When the wrapped command terminates by signal, span status is unconditionally `ERROR` with description `killed by signal SIG<NAME>`, the helper calls `recordException` on a synthetic `Error` capturing the signal name, and the explicit `--status` argument is **ignored** in this case — signal termination is an external interrupt that takes precedence over the user's exit-code-based status assertion. Force-flush and shut down the SDK with the same 5-second deadline as the parent (§10). Exporter failures during force-flush/shutdown are silently swallowed by the standard failure-isolation rule (§10) — the helper does not surface them on stderr or in the exit code. **Signal handling:** the helper does not create a separate process group for the wrapped command (see "Process group" below); SIGINT / SIGTERM delivered to the helper's process group reach the wrapped command via normal POSIX signal delivery, and the helper installs handlers that defer the helper's own termination until the wrapped command has exited, then closes the span, force-flushes, shuts down the SDK, and exits.

  **Fall-through mode** — otherwise (otel disabled, no `TRACEPARENT` in env, `TRACEPARENT` present but malformed — does not parse as a valid W3C trace-context string — or span-mode SDK init failed per "Helper SDK init failure" below): the helper spawns `<command>` as a child process and proxies it. No SDK init (or no successful SDK init), no span overhead, no warning. The helper neither injects nor strips `TRACEPARENT` / `TRACESTATE`; any inherited values pass through to the wrapped command unchanged. The helper preserves the wrapped command's exit code byte-for-byte on normal exit, and uses `128 + signal_number` when the wrapped command terminates by signal — matching the span-mode rule above and the loopx-wide shell convention. It inherits stdin / stdout / stderr from its parent unchanged. As in span mode, the helper does not create a separate process group for the wrapped command; SIGINT / SIGTERM delivered to the shared process group reach the wrapped command directly, and the helper installs handlers that defer its own termination until the wrapped command has exited, then exits with `128 + signal_number`. The helper does **not** attempt POSIX same-process replacement (`execve`) — v1 deliberately accepts one extra process in the tree and the helper's own PID sitting between caller and wrapped command, in exchange for portability across Node and Bun without native interop. The fall-through contract preserves what scripts typically rely on (exit code, stdout / stderr, signal delivery) but not PID identity or exact process-tree shape; scripts that depend on the latter should not wrap with `loopx otel span` in fall-through-prone contexts. Malformed-`TRACEPARENT` parity with the parent SDK: §4 already silently treats malformed inherited `TRACEPARENT` as absent.

  In both modes, helper stdin/stdout/stderr are inherited from the helper's parent unchanged.

  **Process group.** `loopx otel span` does **not** create a separate process group for the wrapped command in either mode; the wrapped command runs in the helper's own process group. This preserves SPEC §7.3's orphan-prevention guarantee end-to-end: when parent loopx escalates to SIGKILL on the active script's process group, that signal reaches the wrapped command alongside the helper, and a wrapped command cannot survive after the helper is killed. No explicit signal forwarding is performed; signals delivered to the script's process group reach the helper and the wrapped command simultaneously via normal POSIX delivery.

  **Helper-side termination guarantees.** All span-mode behaviors specified above — closing the span with the wrapped command's exit code or signal-derived status, calling `recordException` on signal termination, per-helper SDK `forceFlush()` / `shutdown()` — depend on the helper remaining alive long enough to observe the wrapped command's exit. If the helper is force-terminated (e.g., SIGKILL, OOM kill), pending telemetry for that helper invocation is lost; because the helper and the wrapped command share a process group, the same signal has been delivered to the wrapped command, so SPEC §7.3's orphan-prevention guarantee is unaffected.

  **Helper SDK init failure.** When span-mode preconditions are met (`LOOPX_OTEL_ENABLED=true` and a valid `TRACEPARENT` in env) but SDK initialization fails, the helper falls through to fall-through-mode behavior for the rest of that invocation: no span is opened, the inherited `TRACEPARENT` / `TRACESTATE` pass through to the wrapped command unchanged (the helper does **not** inject anything), and the wrapped command is spawned and awaited as in fall-through mode. No stderr warning fires — CLI helpers are short-lived and may be invoked many times within a script; per-call warning would be noisy. Users validate config via `loopx otel test`. The same rule applies to `loopx otel counter` / `histogram` when their enable bit is present but SDK init fails: the data point is silently dropped, the helper exits 0, no warning.
...
`--attr k=v` may repeat. The argument is split on the **first `=`** in the token: everything before that `=` is the key, everything after is the value (so `--attr msg=a=b` records the value `a=b`). The `k` portion is validated against `[A-Za-z_][A-Za-z_0-9.]*`; **an attribute whose key does not match the pattern is silently dropped** (the data point or span is recorded with the remaining valid attributes; no stderr warning is emitted, no exit-code change). This matches the JS/TS helper's invalid-key handling (§3) and the reserved-`loopx.*` rule (§12) — a single normative behavior across all attribute surfaces. The `v` portion is recorded as a string attribute (no type inference in v1). Empty values are allowed: `--attr key=` records an empty-string attribute. A `--attr` argument with no `=` (e.g., `--attr key`) is a flag-syntax error and exits 1 — distinct from the silent-drop rule above, which applies only after the flag has parsed into a key/value pair. Unrecognized flags and missing required arguments are usage errors (exit code 1) regardless of enabled state.
...
- `counter` / `histogram` use the SDK's meter; metric instruments are lazily created and cached by **`(kind, name)`** within the process meter (standard OTel behavior — `counter("x")` and `histogram("x")` produce two distinct instruments and do not conflict). Different attribute sets recorded against the same `(kind, name)` share one instrument.
...
**Helper-driven SDK init failure.** When a telemetry-producing helper call (`withSpan`, `counter`, or `histogram`) attempts SDK initialization and that initialization fails, the offending call falls back to its no-op semantics for that call (`withSpan` invokes `fn` with a stub span; `counter` / `histogram` return without recording). A single stderr warning is emitted per process — at most one across all helper-driven init attempts — identifying the failure. **Helper-driven init failure is sticky for the process:** subsequent helper calls do not retry SDK init and continue to no-op. A subsequent `run()` / `runPromise()` invocation in the same process whose effective `LOOPX_OTEL_ENABLED` is true still attempts SDK init separately (per §10's per-run retry rule); a successful run-driven init thereafter enables in-process helpers for the remainder of the process lifetime, sharing the per-process SDK-init-failure-warning dedup with helper-driven init.
...
When the run is **otel active** (per §1 — effective enabled and SDK init succeeded), the loopx process emits the following span hierarchy automatically:
...
These three are the only span events emitted automatically by loopx during iteration in v1. Other warning conditions surfaced during iteration — including cross-workflow-resolution warnings beyond the version-mismatch case (e.g., a destination workflow whose `package.json` is missing or malformed when the version check would otherwise apply) — remain stderr-only and are not represented as span events. Errors during goto resolution (invalid target syntax, missing target workflow / script) are reflected in iteration **status** (`ERROR`) and `terminal_outcome` per the existing rules, not via separate events.
...
When the run is **otel active** (per §1), loopx injects `TRACEPARENT` (W3C Trace Context) and, when non-empty, `TRACESTATE` into every child script's environment, scoped to the current `loopx.script.exec` span. This is at the **loopx-injected protocol-variable tier** of SPEC §8.3 — it overrides any user-supplied value of the same name. This injection happens regardless of `LOOPX_OTEL_PROPAGATE_TO_SCRIPTS`; that flag governs only `OTEL_*` / `LOOPX_OTEL_*` propagation, not trace linkage.

When the run is not otel active — either because telemetry is effectively disabled or because SDK init failed (per §1, §10) — loopx neither injects nor strips `TRACEPARENT` / `TRACESTATE`; values supplied through any non-protocol environment tier (`RunOptions.env`, local env file, global loopx env, or inherited shell — tiers 2 and 4–6 of §9) merge normally per SPEC §8.3 and reach children unchanged. This preserves the case where loopx is itself called from an outer-process otel context, and aligns with the strict default-off byte-identity guarantee in §1 (loopx adds no behavior of its own when telemetry is not active).

`TRACEPARENT` and `TRACESTATE` are added to SPEC §13's reserved-name table as **conditionally script-protocol-protected**: protected when the run is otel active, and unreserved otherwise. A run whose effective enable bit is `true` but whose SDK init fails leaves `TRACEPARENT` / `TRACESTATE` unreserved for that run — loopx is not injecting them at the protocol tier, so lower-tier values can flow through unchanged.

**`OTEL_PROPAGATORS` does not gate child-env injection.** loopx's injection of `TRACEPARENT` / `TRACESTATE` into child script environments is **mandatory whenever the run is otel active**, regardless of the value of `OTEL_PROPAGATORS`. `OTEL_PROPAGATORS` configures which propagators the SDK uses for inbound and outbound propagation at the SDK level; it does not control loopx's protocol-tier child-env injection. A user who sets `OTEL_PROPAGATORS=baggage` (excluding tracecontext) — or any other value that excludes `tracecontext` — still gets `TRACEPARENT` / `TRACESTATE` written into every child script's environment. The trace-linkage guarantee is independent of `OTEL_PROPAGATORS`. The serialization format used to write `TRACEPARENT` is always W3C tracecontext.
...
Emitted unconditionally when the run is **otel active** (per §1 — effective enabled and SDK init succeeded), subject to standard OTel suppression via `OTEL_METRICS_EXPORTER=none`. A run that is effectively enabled but whose SDK init fails emits no metrics.
...
- `loopx.iteration.count` increments **once per spawn attempt**, matching the `loopx.iteration` span ordinal in §4 — including spawns that terminate before their stdout is parsed (spawn failure, non-zero exit, signal termination, abort, or consumer cancellation while a child is active). The `outcome` attribute on both `loopx.iteration.count` and `loopx.iteration.duration` **mirrors the `loopx.iteration` span status (§4)**: `ok` iff the iteration span closed `OK` — i.e., stdout parsed **and** transition / terminal-decision resolution completed without error; `error` for every other terminal path including non-zero child exit, spawn failure, **invalid `goto` target syntax, missing `goto` target workflow / script during goto resolution**, signal termination, abort, and consumer cancellation while a child is active. Invalid-`goto` and missing-target outcomes therefore count as `error` for both the span status and the metric outcome — the two cannot diverge. The sum of `loopx.iteration.count` across a run equals the largest `loopx.iteration` ordinal, which may exceed `loopx.run.iteration_count` — the latter is the SPEC §7.1 counter that increments only after successful stdout parse, so an iteration whose stdout parsed but whose `goto` was invalid still increments `loopx.run.iteration_count` while contributing an `error` outcome to both `loopx.iteration.count` and `loopx.iteration.duration`.
...
When the run is **otel active** (per §1 — i.e., effective `LOOPX_OTEL_ENABLED` is `true` *and* SDK initialization has succeeded) **and** `LOOPX_OTEL_PROPAGATE_TO_SCRIPTS` is `true`, the run's resolved otel configuration is merged into child script environments at a new tier inserted between `RunOptions.env` (existing tier 2) and the local env file (existing tier 3). Updated SPEC §8.3 precedence (highest wins):

1. **loopx-injected protocol variables** — `LOOPX_BIN`, `LOOPX_PROJECT_ROOT`, `LOOPX_WORKFLOW`, `LOOPX_WORKFLOW_DIR`, `LOOPX_TMPDIR`, plus `TRACEPARENT` / `TRACESTATE` when the run is otel active.
2. **`RunOptions.env`** (programmatic API).
3. **Resolved otel configuration** — only when the run is otel active and propagation is on (see resolution rules below).
...
- All recognized `LOOPX_OTEL_*` knobs **explicitly present in the otel config file** (e.g., `LOOPX_OTEL_CAPTURE_RESULT`, `LOOPX_OTEL_PROPAGATE_TO_SCRIPTS` itself). **Defaulted values for unspecified recognized knobs are not synthesized into tier 3:** a child whose parent's config file does not set `LOOPX_OTEL_CAPTURE_RESULT` sees no `LOOPX_OTEL_CAPTURE_RESULT` in tier 3, even though the parent applies the default `none` for its own recording behavior. Child-side helpers that consume these knobs apply the same documented defaults from §1's recognized-keys table when the env var is absent, so child behavior is identical either way; the rule keeps tier 3 minimal and makes child env introspection match what the user actually configured. `LOOPX_OTEL_*` values supplied via `options.otel` are not in scope for v1 — only `enabled` and `parentContext` are programmatically settable; the capture-mode knobs come from the file or are at default.
...
- **Normalized exporter values for tier 3.** When `OTEL_TRACES_EXPORTER` or `OTEL_METRICS_EXPORTER` resolves (after merging file and inherited env per the rule above) to an unsupported value (anything other than `otlp` or `none`), tier 3 receives the **post-fallback** value `otlp`, **not** the raw user-supplied value. This extends the structured-output protocol protection (per §10's exporter-fallback rationale) across the whole process tree — child-side helpers (`loopx otel counter` / `histogram` / `span`, JS/TS `withSpan` / `counter` / `histogram`, and agent CLIs that read OTel env, e.g. `claude code`) initialize against `otlp` and cannot accidentally write telemetry to a script's stdout. The single per-run-init stderr warning for the offending key (per §10) still fires; the file's raw value remains visible via `loopx otel list`. All other `OTEL_*` keys are propagated as resolved (file-wins-over-inherited) without normalization. The tier-2-overrides-tier-3 rule above is unchanged: a programmatic caller who sets `OTEL_TRACES_EXPORTER=console` in `RunOptions.env` is opting in explicitly and that raw value reaches the child unchanged.
...
**When tier 3 is empty:** any of (a) effective `LOOPX_OTEL_ENABLED` is `false`, (b) `LOOPX_OTEL_PROPAGATE_TO_SCRIPTS` is `false`, (c) otel is disabled outright, or (d) effective enabled is `true` but SDK init failed (the run is not otel active). In all four cases, no `OTEL_*` / `LOOPX_OTEL_*` are propagated by loopx. Trace linkage (`TRACEPARENT` / `TRACESTATE`) is still injected at tier 1 when the run is otel active, regardless of propagation mode; when the run is not otel active, no `TRACEPARENT` is injected (an inherited value passes through via tier 6).
...
- Deferred until the iteration phase begins. `loopx version`, `loopx env *`, `loopx output`, `loopx otel list`/`set`/`enable`/`disable`/`show`, `loopx -h`, `loopx run -h`, and **`loopx install` in all forms** do not initialize the SDK. **`loopx output` is explicitly OTel-dormant**: because it writes the structured-output JSON protocol to stdout (SPEC §6.4), it does not read the otel config file, does not initialize the SDK, and does not emit telemetry, regardless of `LOOPX_OTEL_ENABLED`. `loopx install` (including the post-commit `npm install` pass and `.gitignore` synthesis under SPEC §10.10) is out of scope for telemetry — the only `loopx otel ...` command that initializes the SDK eagerly is `loopx otel test`.
...
- `-n 0` / `maxIterations: 0` early-exit (per SPEC §7.1 step 4) is OTel-dormant: no config-file read, no SDK initialization, no `loopx.run` span, no metrics, no `TRACEPARENT` injection, no OTel-related stderr warnings. The pre-iteration sequence's terminal step is tmpdir creation (SPEC §7.1 step 6), and `-n 0` exits before that step, so the deferred-init trigger above is never reached. Holds even when `LOOPX_OTEL_ENABLED=true` in the otel config file or `options.otel.enabled=true` is supplied.
- SDK initialization failure is non-fatal: a single stderr warning is emitted (per the warning policy below), the run proceeds with telemetry disabled for its lifetime, and exit code is unaffected. The run is not **otel active** (per §1) for the rest of its lifetime — every behavior gated on otel active is suppressed: no `loopx.*` spans/metrics, no per-run flush or shutdown, no `TRACEPARENT` injection at tier 1, no tier-3 OTel-config propagation, no `LOOPX_OTEL_ENABLED=true` reaching children. Children's environments are computed identically to a default-off run; user-supplied lower-tier values still flow through unchanged.
...
- **Process-exit shutdown.**
  ...
  - **Programmatic API host process:** loopx installs **only a `beforeExit` hook** for best-effort SDK `shutdown()` on natural process exit. **No SIGINT/SIGTERM signal handlers are installed by loopx** — initializing the SDK does not change the host process's signal semantics, default Node termination is preserved, and any host-installed signal handlers run unchanged. `process.on('exit')` is also not used (Node disallows async work there). Per-run `forceFlush()` is therefore the only guaranteed export pathway for programmatic invocations on signal-driven exit; host programs that exit on signal, or that `process.exit()` synchronously after `runPromise()` settles, may lose pending exports beyond what `forceFlush()` already wrote. Hosts that need shutdown-on-signal install their own handler that awaits `flushOtel()` (or otherwise drives shutdown) before terminating.
...
**Terminal ordering (normative).** For a run that became **otel active** (effective enabled `true` and SDK init succeeded — per §1), the per-run terminal sequence runs in this exact order; no step is skipped or reordered:

1. The active child process (if any) has exited; its `loopx.script.exec` span is closed.
...
For a run that was effectively enabled but whose SDK init failed (or was effectively disabled), no `loopx.*` spans exist for the run. The underlying child / output / tmpdir steps still run unchanged: the child process exits, output is parsed, transition is resolved, and `LOOPX_TMPDIR` cleanup runs — exactly as on a default-off run. The OTel-specific steps — closing `loopx.script.exec` / `loopx.iteration` / `loopx.run` spans, the `loopx.tmpdir.cleanup_warning` metric increment, per-run `forceFlush()`, and (CLI) SDK `shutdown()` — are all skipped on these paths; there is no span to close, no metric to queue, no flush or shutdown to perform.
...
For programmatic invocations (`run()` / `runPromise()`), step 6 does not execute on settlement; SDK shutdown is deferred to the `beforeExit` hook (loopx installs no signal hooks in programmatic mode — see "Process-exit shutdown" above). `loopx.tmpdir.cleanup_warning` metrics emitted in step 3 are therefore exported reliably for both CLI and programmatic invocations because step 5 always runs before settlement / exit (when the consumer drives the generator to settlement under `run()`).
...
- invalid `LOOPX_OTEL_*` value: one per offending key; fallback to default. **Excludes `LOOPX_OTEL_ENABLED`**, which is permissive per §1's table — any non-enabling value disables silently with no warning.
- SDK-side warnings for invalid `OTEL_*` values surfaced at startup.
- unsupported `OTEL_TRACES_EXPORTER` / `OTEL_METRICS_EXPORTER` value: **at most once per offending key per process**, fallback to `otlp`. Tier-3 child env normalization (§9) re-evaluates the otel config file on every run, so a config-file mutation that introduces or changes an offending value during the lifetime of a long-lived host process is **silently re-normalized** for subsequent runs after the first warning fires; the parent SDK's exporter remains at the value it was initialized with (per the process-wide-SDK rule in §10), but children of the later run still receive the post-fallback `otlp` in their tier-3 env.
...
**`TRACEPARENT` and `TRACESTATE`** are added to SPEC §13's reserved-name table as conditionally script-protocol-protected: protected when the run is **otel active** (per §1 — effective enabled and SDK init succeeded), unreserved otherwise. They are not protocol variables of the loopx core; they exist only when the run is otel active. A run that is effectively enabled but whose SDK init fails leaves them unreserved for that run, since loopx is not injecting them at the protocol tier.
...
**`loopx.*` attribute namespace** is reserved. User code emitting attributes via `withSpan()` / CLI `--attr` flags must not set `loopx.*` keys. Conflicting attributes are **silently dropped** by the helpers; no warning is emitted. (Single normative behavior, not implementation-defined.) The `loopx otel test` probe is exempt from this reservation — its `loopx.version` and `loopx.test_id` attributes are emitted by the loopx subsystem itself per §2.

**`loopx.*` metric instrument names** are similarly reserved. User code creating metrics via `counter()` / `histogram()` (CLI or JS/TS) may not use the `loopx.` prefix. The metric is silently suppressed; a single stderr warning is emitted with the cardinality below. The `loopx otel test` probe instrument (`loopx.otel.test`) is exempt from this reservation per §2:
...
**Reservation warnings are gated by `enabled`.** The reserved-name warnings above (and the invalid-`SpanKind` warning in §3, and the counter / histogram value-rule warnings in §7) fire **only when the helper would otherwise emit telemetry** — i.e., when the in-process SDK is initialized (JS/TS) or the CLI helper's enable bit is present and SDK init succeeded. When the helper is in no-op or fall-through mode (telemetry disabled, CLI helper init failed), the offending call returns silently with no warning. This matches §10's general rule that OTel-related warnings fire only when telemetry is effectively enabled.

**Other invalid OTel instrument names.** Names that are not `loopx.*`-reserved but still fail OTel's standard instrument-name validation (e.g., names not matching the OTel pattern, names exceeding length limits, characters disallowed by the SDK) fall under the SDK's standard behavior — typically silent rejection at instrument creation. Loopx adds no separate validation layer or warning for these beyond the reserved-`loopx.*` rule above.
...
- **§4.2 (Options)** — under `loopx otel` parsing rules, define `--attr k=v` repetition and the first-`=` split rule (§2 of this ADR), the attribute-key validation pattern `[A-Za-z_][A-Za-z_0-9.]*` with the **silent-drop on invalid key** rule (no stderr warning, no exit-code change — matches the JS/TS helper and the reserved `loopx.*` rule per §2 / §3 / §12 of this ADR), the distinction that a `--attr` argument with no `=` is a flag-syntax usage error (exit 1), `--status ok|error` for `loopx otel span`, the `-h` / `--help` short-circuit on each `loopx otel` subcommand, and the duplicate-flag / unknown-flag rules consistent with the `run` and `install` scopes.
- **§4.3 (Subcommands)** — add `loopx otel` and all its sub-subcommands with synopses. Specify that **`loopx otel list` is the parsed config-file-only view** (sorted lexicographically, no inherited `OTEL_*`, no redaction; semantically invalid recognized `LOOPX_OTEL_*` values shown verbatim — fallback / default behavior is shown by `loopx otel show`); per §2 of this ADR. Specify the credential-redaction rule for `loopx otel show` (redacts values for keys whose names contain `HEADERS`, `TOKEN`, `AUTH`, `PASSWORD`, `SECRET`, `CREDENTIAL`, or `BEARER`; `loopx otel list` remains the unredacted view). Specify that **`loopx otel set <name> <value>` does not validate values semantically at set time** — values are written verbatim and the per-key invalid-value warning fires at SDK init / `show`, not at `set` (per §2 of this ADR). Specify the duplicate-key rewrite rule for `enable` / `disable` / `set` / `remove` (per §2 of this ADR — `enable` / `disable` / `set` collapse all occurrences of the managed key to a single occurrence at the position of the first existing occurrence; `remove` deletes every occurrence; unrelated lines including unrelated duplicate keys are preserved). Specify that **`loopx otel span ... -- <command>` runs the wrapped command in the helper's own process group** (no separate process group is created in either span or fall-through mode), so SPEC §7.3's process-group SIGKILL escalation reaches the wrapped command alongside the helper and the orphan-prevention guarantee is preserved end-to-end (§2 of this ADR). No explicit signal forwarding is performed; signals delivered to the script's process group reach the helper and the wrapped command via normal POSIX delivery. Specify the disabled-state short-circuit for `loopx otel test`: when otel is disabled in config, `test` exits 1 with the disabled message and does **not** validate or warn on other invalid `LOOPX_OTEL_*` / `OTEL_*` keys; use `loopx otel show` for validation independent of enable state (§2 of this ADR). The helper's span-status / `recordException` / `forceFlush()` / `shutdown()` guarantees apply only when the helper remains alive to observe the wrapped command's termination — a force-terminated helper (e.g., SIGKILL) cannot perform these and pending telemetry for that helper invocation is lost; the wrapped command, sharing the process group, has been delivered the same signal. Specify that **span-mode SDK init failure falls through to fall-through mode** (preserves inherited `TRACEPARENT` / `TRACESTATE` unchanged; no warning) and that `counter` / `histogram` SDK init failure silently drops the data point (no warning); CLI helpers are short-lived and per-invocation warnings would be noisy, so users validate via `loopx otel test`. Add `loopx otel -h` and per-subcommand `loopx otel <sub> -h` to the help-flag short-circuit pattern.
- **§5.4 (Validation Scope)** — add a row for `loopx otel *`: does not require `.loopx/`, performs no discovery, no validation.
- **§6.4 / §6.5** — add a cross-reference to the new observability chapter. Clarify that `output()` / `input()` continue to be the only stdout-protocol helpers; `counter` / `histogram` (CLI and JS/TS) export only via OTLP and write nothing to stdout, while `loopx otel span ... -- <command>` inherits the wrapped command's stdout (which therefore can still affect the script's structured-output stdout — wrapping a command that produces structured stdout requires the same care as any other process-substitution pattern). **`output()` semantics in SPEC §6.4 are unchanged** — it does not flush the OTel SDK; scripts that need guaranteed export before `output()` use the new `flushOtel()` helper (§3 of this ADR) per the documented `await flushOtel(); output(...)` pattern. **`loopx output` is explicitly OTel-dormant**: because it writes the structured-output JSON protocol to stdout, it does not read the otel config file, does not initialize the SDK, and does not emit telemetry, regardless of `LOOPX_OTEL_ENABLED` (§10 of this ADR).
- **§7.1 (Basic Loop)** — insert SDK initialization between tmpdir creation (current step 6) and the first child spawn (current step 7), **conditional on effective `LOOPX_OTEL_ENABLED`** (per §1). At terminal outcome, insert (in order): `LOOPX_TMPDIR` cleanup → close `loopx.run` span → per-run `forceFlush()` (5-second deadline) → CLI-only SDK `shutdown()` (5-second deadline) → CLI exit (§10's terminal-ordering note in this ADR). **`LOOPX_TMPDIR` cleanup remains unconditional** for runs that created a tmpdir — no change to its existing semantics; it is part of normal loop teardown regardless of OTel state. The OTel-specific terminal steps — `loopx.tmpdir.cleanup_warning` metric emission during cleanup, closing `loopx.run`, per-run `forceFlush()`, and CLI SDK `shutdown()` — run **only when the run is otel active** (effective enabled and SDK init succeeded — per §1 / §10). A run whose effective enable bit is `true` but whose SDK init fails still runs `LOOPX_TMPDIR` cleanup unchanged; the OTel-specific steps are skipped (no spans to close, no flush, no shutdown). Per-run `forceFlush()` runs for both CLI and programmatic invocations; SDK `shutdown()` runs at process exit only. For `run()` (async generator), tmpdir cleanup, `loopx.run` close, and per-run `forceFlush()` run when the generator is **driven to settlement**, not when the final terminal `Output` is yielded — preserving the existing SPEC §7.4 / §9.1 rule that the final yield is not itself settlement. Iteration-level spans (`loopx.iteration`, `loopx.script.exec`) close when their iteration ends and are not deferred to settlement. The existing step 4 (`-n 0` / `maxIterations: 0` early-exit) remains **OTel-dormant** — it occurs before tmpdir creation at step 6, so the SDK-init insertion at step 6→7 never triggers; no config-file read, no SDK init, no `loopx.run` span, and no OTel-related warnings on this path even when telemetry would otherwise be effectively enabled (§1, §10).
- **§7.2 (Error Handling)** — add SDK initialization failure as a non-fatal warning. Specify that an init failure disables telemetry only for the run that triggered it; subsequent runs in the same process whose effective `LOOPX_OTEL_ENABLED` is true retry SDK init, and the SDK-init-failure stderr warning is deduped to at most once per process across all such retries. Specify that **helper-driven SDK init failure (JS/TS)** uses the same per-process dedup as run-driven init failure: a single warning across the entire process, and the offending helper call falls back to its no-op semantics; helper-driven init failure is sticky for the process (subsequent helper calls do not retry SDK init), but a later `run()` / `runPromise()` is permitted to retry init separately and a successful run-driven init thereafter enables in-process helpers. Specify that **CLI runtime helpers** (`loopx otel span` span mode, `counter`, `histogram`) on SDK init failure fall through silently — no warning, no telemetry; `span` falls through to fall-through mode (preserves inherited `TRACEPARENT` / `TRACESTATE` unchanged), `counter` / `histogram` drop the data point and exit 0. Add SDK process-exit shutdown timeout as a non-fatal warning. Add the unsupported-exporter fallback warning (§10) — **at most once per offending key per process**, with subsequent runs that re-observe the same offending value silently re-normalizing tier-3 child env (§9 / §10 of this ADR) — the counter / histogram value-rule violation warning (§7), and the explicit JS/TS `flushOtel()` timeout warning (§3 / §10) to the non-fatal warning list. All OTel-related warnings fire only when telemetry is effectively enabled (or when the helper would otherwise emit telemetry, for helper-side warnings — see §12); a default-off run produces no OTel-related stderr, and a CLI helper in no-op or fall-through mode produces no OTel-related stderr.
- **§7.3 (Signal Handling)** — clarify that, on **CLI** signal-driven exit, loopx's signal hook runs the §10 terminal-ordering sequence (tmpdir cleanup → close `loopx.run` → per-run `forceFlush()` → SDK `shutdown()`) before the signal-exit. The two OTel deadlines (per-run `forceFlush()` and SDK `shutdown()`) run **sequentially**, not concurrently: per-run `forceFlush()` has its own 5-second deadline, and SDK `shutdown()` has its own 5-second deadline that begins after `forceFlush()` returns or its deadline elapses. Worst case is therefore 5 + 5 = 10 seconds of OTel work between the signal handler firing and the actual process exit. Either deadline returning early proceeds immediately to the next step; neither "runs concurrently with the process exit" — both run before it. The 10-second total is bounded; loopx introduces no additional waits beyond these two deadlines for OTel cleanup. **Programmatic API host process:** loopx installs **only a `beforeExit` hook** for natural-exit best-effort SDK shutdown. **No SIGINT/SIGTERM signal handlers are installed by loopx in programmatic mode** — initializing the SDK does not change the host process's signal semantics, default Node termination is preserved, and any host-installed signal handlers run unchanged. `process.on('exit')` is also not used because Node disallows async work in that hook. Per-run `forceFlush()` is therefore the only guaranteed export pathway for programmatic invocations on signal-driven exit; hosts that need shutdown-on-signal install their own handler that awaits `flushOtel()` (or otherwise drives shutdown) before terminating. Specify that **`loopx otel span` runs the wrapped command in the helper's own process group** (no separate process group is created), so the SPEC §7.3 process-group SIGKILL escalation reaches the wrapped command alongside the helper and the orphan-prevention guarantee is preserved end-to-end (§2 of this ADR).
...
- **§8.1 (Global Storage)** — add a sibling subsection for the new `$XDG_CONFIG_HOME/loopx/otel` config file, with the same fallback rules, concurrent-mutation caveats, the `0600` permission requirement on writes, and the read-side rule that loopx neither chmods nor warns about existing-file permissions. Document the runtime-side rule per §1's strict default-off behavior — including the explicit scope clarification that "byte-identical" refers to loopx-visible **protocol behavior** (stdout, stderr, exit code, thrown / rejected outcome, child env injection) and does not promise an identical filesystem syscall profile or wall-clock timing (§1 of this ADR) — and the rule that an unreadable file is silently treated as absent → disabled when telemetry is not effectively enabled, and is a non-fatal one-shot warning only when `options.otel.enabled=true` forces telemetry on despite the unreadable file. The contrasting config-management-command rule is that an unreadable file is fatal (§2 of this ADR). Document the **parse-level vs semantic-level error separation** explicitly per §2 of this ADR: **parse-level errors** are lines without `=` (mirroring SPEC §8.1's existing rule for the global env file) and env-var names not matching `[A-Za-z_][A-Za-z0-9_]*`. They are warn-and-ignore at every read (one stderr warning per offending line, line otherwise skipped) on both the runtime path and any config-management command that reads the parsed file (`list`, `show`, `test`, and the rewrite commands `enable` / `disable` / `set` / `remove` when they read before writing). **Recognized-key value validation is semantic-level, not parse-level**: a recognized `LOOPX_OTEL_*` key whose value does not satisfy the per-key validation in §1's recognized-keys table is parsed successfully, the line is retained verbatim at the file level, and the value-fallback warning is deferred. The behavior matrix is (per §2 of this ADR): `loopx otel list` shows the raw configured value with no fallback and no warning; `loopx otel set` writes the supplied value verbatim with no semantic validation; `enable` / `disable` / `remove` preserve the offending line unchanged at rewrite time with no warning attributable to the offending key; `loopx otel show` displays the post-fallback effective value and emits the per-key warning at most once per offending key per process; `loopx otel test` applies fallback + warning at SDK init when enabled, but its disabled-state check short-circuits all such validation when otel is disabled in config; the runtime path applies fallback + warning at SDK init when the run is otel active. `enable` / `disable` / `set` / `remove` rewrites preserve unchanged lines (including invalid ones beyond the key being managed) and apply the duplicate-key rewrite rule from §2 (collapse-on-write for `enable` / `disable` / `set`, delete-all for `remove`). Recognized-keys table from §1 of this ADR is included; in particular `LOOPX_OTEL_CAPTURE_RESULT_TRUNCATE_BYTES=0` and `LOOPX_OTEL_CAPTURE_STDIN_TRUNCATE_BYTES=0` are valid values that omit the corresponding preview attributes (no zero-length strings recorded), matching the omit-when-empty rule for the SHA-256 attributes. Disk-read timing for the otel config file: after the pre-iteration sequence completes successfully, for all invocation modes (per §1) — and never for `-n 0` / `maxIterations: 0` early-exit, which is OTel-dormant per §1 / §10 of this ADR. The inherited `OTEL_*` / `TRACEPARENT` / `TRACESTATE` / `XDG_CONFIG_HOME` / `HOME` values consulted alongside the file are captured by the **existing** inherited `process.env` snapshot the section already specifies — no new inherited-env snapshot point is introduced. Implementations must compute SDK configuration from this snapshot plus the otel config file snapshot; live `process.env` mutations after the snapshot do not affect SDK initialization for that run (this matters most for `runPromise()`, where the snapshot is at the call site but SDK init happens asynchronously after pre-iteration).
- **§8.3 (Injection)** — replace the precedence list with the 6-tier list in §9 of this ADR. Add `TRACEPARENT` / `TRACESTATE` to the protocol-variable table with the "only when the run is **otel active**" qualifier (per §1 / §5 of this ADR — effective enabled and SDK init succeeded; a run that is effectively enabled but whose SDK init fails leaves them unreserved for that run). Note that the otel config tier (3) sits **above** the local env file (4), the global loopx env (5), and inherited env (6) by deliberate design — the global otel config is the canonical OTel source for a run; lower tiers cannot silently shadow it. This inverts the standard "local beats global" pattern that applies among tiers 4–6 for non-OTel keys, and is intentional. CLI users override per-project by editing the otel config file; programmatic callers use `RunOptions.env` (tier 2). Tier 3 is empty when the run is not otel active (effective disabled, propagation off, or SDK init failed); see §9 of this ADR for the full set of conditions. Tier 3 contains only recognized `LOOPX_OTEL_*` knobs **explicitly present in the otel config file** — defaulted values for unspecified knobs are not synthesized into tier 3 (per §9 of this ADR). Tier 3 propagates the **post-fallback (normalized)** value of `OTEL_TRACES_EXPORTER` / `OTEL_METRICS_EXPORTER` — when the resolved value is unsupported (anything other than `otlp` or `none`), children receive `otlp`, not the raw user-supplied value (§9 of this ADR). All other `OTEL_*` keys are propagated as resolved without normalization.
...
- **§13** — add `TRACEPARENT`, `TRACESTATE` rows (conditionally script-protocol-protected when the run is **otel active** per §1 of this ADR — effective enabled and SDK init succeeded; a run that is effectively enabled but whose SDK init fails leaves them unreserved for that run). Add a paragraph reserving the `loopx.*` attribute and metric-instrument-name namespaces, with the explicit exemption that `loopx otel test`'s probe span / metric (the `loopx.otel.test` instrument and span name, plus the probe's `loopx.version` and `loopx.test_id` attributes) is part of the loopx subsystem and is not subject to the user-attribute or user-metric reservation (§2, §12 of this ADR). Specify that **reservation warnings (and the invalid-`SpanKind` warning, and the counter / histogram value-rule warnings) are gated by the helper actually emitting telemetry** — they fire only when the in-process SDK is initialized (JS/TS) or the CLI helper's enable bit is present and SDK init succeeded; in no-op or fall-through mode the offending call returns silently with no warning (§12 of this ADR). Specify that **other invalid OTel instrument names** beyond the reserved `loopx.*` prefix fall under the SDK's standard validation behavior — typically silent rejection at instrument creation; loopx adds no separate validation layer for these (§12 of this ADR).
...
- All of the above is opt-in. A user who never runs `loopx otel enable` pays zero SDK / exporter / network overhead, no extra env vars in spawned scripts, and byte-identical stdout / stderr / exit behavior compared to a build without this ADR. (For runs that reach the deferred OTel config-read point — i.e., runs where pre-iteration completes successfully and `-n 0` / `maxIterations: 0` does not early-exit — the otel config file is read silently to resolve the enable bit, but this is below the threshold of observable cost. Pre-iteration failures and `-n 0` / `maxIterations: 0` early exits read no config file at all, per §1 / §10.)
