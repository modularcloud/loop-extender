...
Both warnings — for parse-level errors and for invalid-value fallbacks — are subject to the same runtime gating as other otel warnings (per §1's strict default-off rule and §10's warning policy): they fire only when telemetry is effectively enabled for the run, or when a `loopx otel *` config-management command is explicitly invoked. `list` prints parsed config-file-only key/value lines (skipping malformed lines and invalid env-var-name lines with one stderr warning per offending line; semantically invalid recognized `LOOPX_OTEL_*` values remain visible as their raw configured value, per the `list` description above); `show` resolves effective configuration from valid lines plus inherited `process.env` `OTEL_*` (with the same parse-level warnings, plus the per-key invalid-value warning whose post-fallback default is what `show` displays); `test` runs against that effective configuration. `enable`, `disable`, `set`, and `remove` rewrite the file preserving valid lines unchanged; they neither scrub nor rewrite invalid lines beyond the specific key being managed by the command. Unreadable file (above) is fatal for config-management commands; readable-but-invalid is warn-and-ignore.
...
- **`loopx otel span <name> ... -- <command>`** — runs `<command>` as a child process. Two modes:
  ...
  **Span mode** — when otel is enabled (`LOOPX_OTEL_ENABLED=true` or `1` in the helper's env) **and** a valid W3C `TRACEPARENT` is present in the helper's environment. These are the **only** two gating predicates; "reachability" of an `OTEL_*` endpoint is not a gating predicate. The helper initializes its own SDK from whatever `OTEL_*` configuration is reachable in inherited env (including SDK defaults when no explicit endpoint is set), starts a span as a child of the `TRACEPARENT` context, **spawns** the wrapped command as a child process (does not `exec`), injects both `TRACEPARENT` and, when non-empty, `TRACESTATE` derived from the new span's context into the spawned command's environment (mirroring the parent loopx process's child-env injection in §5), awaits exit. **Helper exit code:** when the wrapped command exits normally, the helper exits with the wrapped command's exit code byte-for-byte. When the wrapped command terminates by signal (whether via the helper's own signal forwarding or a signal delivered by an external sender, including SIGKILL), the helper exits with `128 + signal_number`, matching the loopx-wide shell convention used elsewhere in this ADR (§4 `process.exit_code` on `loopx.script.exec`, §7 `loopx.script.exit_code` counter, SPEC §7.3). **Span status:** when the wrapped command exits normally, span status is taken from `--status` if provided, otherwise derived from exit code (0 → OK, non-zero → ERROR). When the wrapped command terminates by signal, span status is unconditionally `ERROR` with description `killed by signal SIG<NAME>`, the helper calls `recordException` on a synthetic `Error` capturing the signal name, and the explicit `--status` argument is **ignored** in this case — signal termination is an external interrupt that takes precedence over the user's exit-code-based status assertion. Force-flush and shut down the SDK with the same 5-second deadline as the parent (§10). Exporter failures during force-flush/shutdown are silently swallowed by the standard failure-isolation rule (§10) — the helper does not surface them on stderr or in the exit code. SIGINT and SIGTERM received by the helper are forwarded to the wrapped command's process group; the helper waits for the command to exit before closing its span and exiting itself.
  ...
  **Process group.** In both span and fall-through modes, `loopx otel span` launches the wrapped command in **its own process group**, and signal forwarding (SIGINT / SIGTERM) targets that process group. This ensures forwarded signals reach the wrapped command and its descendants without affecting the helper itself or unrelated sibling processes. Failure to create the process group is treated as a wrapped-command spawn failure per the next paragraph.

  **Helper-side termination guarantees.** All span-mode behaviors specified above — recording the wrapped command's exit code, deriving span status from exit code or `--status`, recording `recordException` on signal termination, per-helper SDK `forceFlush()` / `shutdown()` — are guaranteed only when the helper itself remains alive long enough to observe the wrapped command's termination. If the helper is force-terminated before the wrapped command exits (e.g., the helper itself receives `SIGKILL`, an OOM kill, or any other signal that bypasses the helper's signal handlers), the helper cannot record span status, call `recordException`, or run `forceFlush()` / `shutdown()`; pending telemetry for that helper invocation is lost and the wrapped command is not guaranteed any orderly cleanup the helper would normally provide.

  **Wrapped command spawn failure.** When `<command>` cannot be launched at all — `ENOENT` (command not found), `EACCES` (permission denied), or any other OS-level launch error (including process-group creation failure) — the helper exits with the standard POSIX shell convention: **127** on `ENOENT`, **126** on `EACCES`, **1** on other launch errors. A one-line diagnostic identifying the failing command is written to stderr; the exact text format is implementation-defined. In span mode, the helper additionally records the failure on its span (sets ERROR status, calls `recordException`), force-flushes, and shuts down the SDK before exiting; exporter failures during the failure-path flush remain silent. In fall-through mode no SDK is initialized; the spawn failure is reported and the helper exits with the codes above.
...
**Terminal ordering (normative).** For a run that became **otel active** (effective enabled `true` and SDK init succeeded — per §1), the per-run terminal sequence runs in this exact order; no step is skipped or reordered. A run that was effectively enabled but whose SDK init failed runs steps 1–3 (with no metric increments at step 3, since the SDK is not active) and skips steps 4–6 (no spans to close, no flush, no shutdown):
