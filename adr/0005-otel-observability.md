...
**Config-file parse-level errors (readable file, invalid content).** Invalid env-var names (lines that parse as a `KEY=VALUE` shape but where the key does not match `[A-Za-z_][A-Za-z0-9_]*`) and invalid `LOOPX_OTEL_*` values are **not fatal** for config-management commands; they mirror the runtime rule in §1. Each offending line emits one stderr warning and is otherwise ignored. `list` prints valid lines and skips invalid ones (with warnings); `show` resolves effective configuration from valid lines plus inherited `process.env` `OTEL_*` (with the same warnings); `test` runs against that effective configuration. `enable`, `disable`, `set`, and `remove` rewrite the file preserving valid lines unchanged; they neither scrub nor rewrite invalid lines beyond the specific key being managed by the command. Unreadable file (above) is fatal; readable-but-invalid is warn-and-ignore.
...
- **`loopx otel span <name> ... -- <command>`** — runs `<command>` as a child process. Two modes:
  ...
  **Wrapped command spawn / exec failure.** When `<command>` cannot be launched at all — `ENOENT` (command not found), `EACCES` (permission denied), or any other OS-level launch error — the helper exits with the standard POSIX shell convention: **127** on `ENOENT`, **126** on `EACCES`, **1** on other launch errors. A one-line diagnostic identifying the failing command is written to stderr; the exact text format is implementation-defined. In span mode, the helper additionally records the failure on its span (sets ERROR status, calls `recordException`), force-flushes, and shuts down the SDK before exiting; exporter failures during the failure-path flush remain silent. In fall-through mode no SDK is initialized; `exec` failure is reported and the helper exits with the codes above without ever launching the command.
...
**Consumer-cancellation cases.** Three sub-cases, all closing `loopx.run` as `OK` with `terminal_outcome = consumer_cancellation`:

1. **Cancellation while a child is active** (`break` mid-iteration, `.return()` / `.throw()` while the child has not yet exited). The active child is killed per SPEC §9.1; the open `loopx.script.exec` and `loopx.iteration` spans both close as `ERROR` with no `loopx.output.*` attributes.
2. **Cancellation between iterations** (after the previous iteration yielded an `Output`, before the consumer requests the next one). No new `loopx.iteration` span is created; the previously-closed iteration retains whatever status it already had (`OK` if its output parsed, `ERROR` otherwise).
3. **Pre-first-`next()` cancellation under `run()`** (per SPEC §9.1 carve-out). No pre-iteration work runs, so SDK initialization never happens and no telemetry is emitted at all — byte-identical to the pre-ADR behavior.
