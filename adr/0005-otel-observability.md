...
- **§4.3 (Subcommands)** — add `loopx otel` and all its sub-subcommands with synopses, including the credential-redaction rule for `loopx otel show` (redacts values for keys whose names contain `HEADERS`, `TOKEN`, `AUTH`, `PASSWORD`, `SECRET`, `CREDENTIAL`, or `BEARER`; `loopx otel list` remains the unredacted view) and the duplicate-key rewrite rule for `enable` / `disable` / `set` / `remove` (per §2 of this ADR — `enable` / `disable` / `set` collapse all occurrences of the managed key to a single occurrence at the position of the first existing occurrence; `remove` deletes every occurrence; unrelated lines including unrelated duplicate keys are preserved). Specify that `loopx otel span ... -- <command>` launches the wrapped command in its own process group and forwards SIGINT / SIGTERM to that group (§2 of this ADR). Add `loopx otel -h` and per-subcommand `loopx otel <sub> -h` to the help-flag short-circuit pattern.
