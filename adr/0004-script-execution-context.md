...
- **No extra canonicalization.** loopx does not apply additional `realpath` / canonicalization to the project-root path beyond what the runtime already provides. For the CLI, `LOOPX_PROJECT_ROOT` is derived from `process.cwd()` at invocation; for the programmatic API, from `RunOptions.cwd` when supplied or `process.cwd()` at call time. Consequently, an explicitly-supplied `RunOptions.cwd` is used verbatim, while a value sourced from `process.cwd()` reflects whatever the runtime's cwd function returns — typically a `getcwd(3)`-style canonical path on POSIX, which is not guaranteed to match the shell's `$PWD` spelling when the user reached the project root through a symlink. loopx does not read `$PWD` and does not attempt to preserve a logical shell spelling that differs from the runtime cwd. Callers who need a specific spelling should pass `RunOptions.cwd` explicitly; callers who need a canonical realpath should compute it themselves. `LOOPX_WORKFLOW_DIR` (§4) is composed from the same project-root value plus the discovery-time `.loopx/<workflow>` suffix and inherits the same "no extra canonicalization by loopx" rule; the `.loopx/<workflow>` suffix itself is preserved as discovered (a symlinked workflow entry keeps the symlinked name).
...
- **Scripts are invoked by absolute discovery-time path.** loopx spawns each script using the absolute path by which it was discovered under `.loopx/` (e.g., `bash /project/.loopx/review-adr/index.sh`), not a path relative to any cwd. For bash scripts this makes `$0` an absolute path and, because bash does not canonicalize `$0`, `$(dirname "$0")` equals `LOOPX_WORKFLOW_DIR` byte-for-byte — including when the workflow directory is reached through a symlink under `.loopx/`. This bash equality is normative.
- **JS/TS `import.meta.url` equality is not guaranteed under symlinks.** For JS/TS scripts the invocation path is likewise absolute, but the main-module URL exposed to the script is produced by the runtime: Node's default behavior for the main module resolves symlinks and exposes the canonical path via `import.meta.url`, so under a symlinked workflow directory `dirname(fileURLToPath(import.meta.url))` can resolve to the symlink target rather than to `LOOPX_WORKFLOW_DIR`. Bun's main-module symlink behavior is not specified to match Node's. loopx does not apply runtime flags (e.g., Node's `--preserve-symlinks-main`) to force symlink preservation for the main module. Consequently, **`LOOPX_WORKFLOW_DIR` is authoritative** for JS/TS code that needs to reach workflow-local assets; the `import.meta.url`-derived dirname equals `LOOPX_WORKFLOW_DIR` only when the workflow directory is not reached through a symlink, and that equality is non-normative beyond that case.
- **Symlink behavior — matches discovery-time path.** If a workflow directory is reached through a symlink (SPEC §5 follows symlinks during discovery), the absolute path used both to invoke the script (per the preceding rules) and to populate `LOOPX_WORKFLOW_DIR` is the symlinked path (`<project>/.loopx/<name>`), not the symlink's realpath target. This keeps `LOOPX_WORKFLOW_DIR` in agreement with bash `$(dirname "$0")` and with any cross-workflow `goto` target string the user wrote. JS/TS `import.meta.url` may canonicalize the symlink per the runtime's main-module rules (see preceding bullet), so a JS/TS script that needs the discovery-time spelling must read `LOOPX_WORKFLOW_DIR` rather than derive from `import.meta.url`.
...
- **§6.1 — Working Directory.** Replace "All scripts run with the workflow directory as their working directory" with "All scripts run with `LOOPX_PROJECT_ROOT` as their working directory". Drop the "ensures relative imports and `node_modules/` resolve naturally" rationale — it is inaccurate, since module resolution is file-relative rather than cwd-relative. Note that scripts remain free to `cd` / `process.chdir()` themselves, that such changes are scoped to the individual child process, and that later script spawns always start at project-root cwd. Record the normative rule — carried by §6.1 alone — that loopx invokes each script using its absolute discovery-time path, which preserves the symlink spelling of any `.loopx/<workflow>` entry reached via a symlink. Cross-reference §6.2 and §6.3 for the language-specific consequences of that invocation rule.
- **§6.2 — Bash Scripts.** Note that the absolute discovery-time invocation path makes `$0` an absolute path, so `$(dirname "$0")` equals `LOOPX_WORKFLOW_DIR` byte-for-byte — including when the workflow directory is reached through a symlink under `.loopx/`.
- **§6.3 — JS/TS Scripts.** Note that `dirname(fileURLToPath(import.meta.url))` equals `LOOPX_WORKFLOW_DIR` when the workflow directory is not reached through a symlink, but is not guaranteed to match under a symlinked workflow directory because the runtime (Node by default; Bun behavior unspecified) may canonicalize the main module's URL. Scripts that need a portable, symlink-preserving workflow-directory handle must use `LOOPX_WORKFLOW_DIR`. loopx does not pass runtime-specific symlink-preservation flags (e.g., `--preserve-symlinks-main`).
...
- Bash: `$(dirname "$0")` computes the script's own workflow directory correctly under project-root cwd, and the computed value equals `LOOPX_WORKFLOW_DIR` byte-for-byte — including when the workflow directory is reached through a symlink under `.loopx/`.
- JS/TS: `dirname(fileURLToPath(import.meta.url))` equals `LOOPX_WORKFLOW_DIR` when the workflow directory is not reached through a symlink. Under a symlinked workflow directory this equality is not required (the runtime's main-module symlink handling applies); tests that need a deterministic value across both symlinked and non-symlinked layouts must read `LOOPX_WORKFLOW_DIR` directly rather than derive it from `import.meta.url`.

### `LOOPX_WORKFLOW_DIR`

- `LOOPX_WORKFLOW_DIR` is injected into every script's environment as an absolute path.
- The path exists and is a directory.
- For a top-level bash workflow script, `LOOPX_WORKFLOW_DIR` equals `$(dirname "$0")` — including when the workflow is reached through a symlink under `.loopx/`.
- For a top-level JS/TS workflow script invoked through a non-symlinked workflow directory, `LOOPX_WORKFLOW_DIR` equals `dirname(fileURLToPath(import.meta.url))`. Under a symlinked workflow directory this equality is not required (Node typically canonicalizes the main module's URL and Bun's behavior is not specified to match); portable tests must read `LOOPX_WORKFLOW_DIR` directly.
