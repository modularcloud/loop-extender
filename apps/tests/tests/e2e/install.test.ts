...
describe("SPEC: Install Command (T-INST-* / ADR-0003 workflow model)", () => {
  ...
  describe("Auto-install of Workflow Dependencies (Spec 10.10)", () => {
    forEachRuntime((runtime) => {
      ...
      // SPEC §10.10 "Signals during the auto-install pass when no
      // npm child is active": the no-active-child contract must
      // hold for the gap between one workflow's npm-child exit and
      // the next workflow's safeguard `lstat`. Uses the test-only
      // LOOPX_TEST_AUTOINSTALL_PAUSE=post-exit-first seam (TEST-SPEC
      // §1.4) to deterministically deliver the signal in that window.
      // T-INST-116j: SIGTERM × successful npm exit × completed
      // node_modules/ side-effect preservation.
      // T-INST-116j2: SIGINT × non-zero npm exit × aggregate-report
      // suppression on the npm-non-zero accumulator path.
      // T-INST-116j3: SIGTERM × non-zero npm exit (parity for j2).
      // ─────────────────────────────────────────────────────────

      it("T-INST-116j: SIGTERM during post-exit-first window — completed node_modules/ preserved, no further processing", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm(
          {
            exitCode: 0,
            createFiles: ["node_modules/completed-marker"],
            logFile,
          },
          async (fake) => {
            const { result, sendSignal } = runCLIWithSignal(
            ...
            // Poll for the marker file until it exists (the seam writes +
            // fsyncs it before the bounded delay begins, so polling is
            // race-free).
            const pauseDeadline = Date.now() + 30_000;
            while (!existsSync(markerPath)) {
              if (Date.now() > pauseDeadline) {
            ...
            expect(["alpha", "beta", "gamma"]).toContain(first);
            expect(marker.processed).toEqual([]);
            const notReached = marker.remaining as string[];
            ...
            // (b) Exactly one shim invocation, against the first-processed
            // workflow.
            const invocations = fake.readInvocations();
            expect(invocations.length).toBe(1);
            expect(invocations[0].cwd.split("/").pop()).toBe(first);

            // (c) The first-processed workflow has a synthesized .gitignore.
            const firstGitignore = join(project!.loopxDir, first, ".gitignore");
            expect(existsSync(firstGitignore)).toBe(true);
            expect(readFileSync(firstGitignore, "utf-8")).toBe("node_modules\n");

            // (d) Other workflows do not have a .gitignore.
            for (const w of notReached) {
              expect(existsSync(join(project!.loopxDir, w, ".gitignore"))).toBe(
                false,
              );
            ...
            // (f) No aggregate failure report on signal-termination path.
            expect(r.stderr).not.toMatch(/auto-install failures/);

            // (g) Completed node_modules/ from the first-processed workflow's
            // just-exited npm child remains on disk per SPEC §10.10's
            // "side effects completed before the signal observation remain
            // on disk" clause.
            expect(
              existsSync(
                join(project!.loopxDir, first, "node_modules", "completed-marker"),
              ),
            ).toBe(true);

            // No node_modules/ for unreached workflows (no npm spawn).
            for (const w of notReached) {
              expect(existsSync(join(project!.loopxDir, w, "node_modules"))).toBe(
                false,
              );
            }
          },
      ...
      it("T-INST-116j2: SIGINT after non-zero npm exit during post-exit-first window — aggregate report suppressed", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm({ exitCode: 1, logFile }, async (fake) => {
          const { result, sendSignal } = runCLIWithSignal(
            ["install", `${gitServer!.url}/multi.git`],
            {
              cwd: project!.dir,
          ...
          // (a) Exit 130 (signal supersedes the npm-non-zero exit code).
          expect(r.exitCode).toBe(130);
          ...
          // (d) Other workflows have no .gitignore.
          for (const w of notReached) {
            expect(existsSync(join(project!.loopxDir, w, ".gitignore"))).toBe(
              false,
            );
          ...
          // (f) No aggregate failure report on stderr — the npm-non-zero
          // accumulator entry was recorded but signal-termination
          // suppression prevents the report from being emitted.
          expect(r.stderr).not.toMatch(/auto-install failures/);
        });
      });

      it("T-INST-116j3: SIGTERM after non-zero npm exit during post-exit-first window — SIGTERM-axis parity", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm({ exitCode: 1, logFile }, async (fake) => {
          ...
          // (a) Exit 143 (signal supersedes the npm-non-zero exit code).
          expect(r.exitCode).toBe(143);
          ...
          // (c) First workflow's .gitignore present.
          const firstGitignore = join(project!.loopxDir, first, ".gitignore");
          expect(existsSync(firstGitignore)).toBe(true);
          expect(readFileSync(firstGitignore, "utf-8")).toBe("node_modules\n");
          ...
          // (f) No aggregate failure report — SIGTERM-axis parity for the
          // npm-non-zero suppression path.
          expect(r.stderr).not.toMatch(/auto-install failures/);
        });
      });

      // ─────────────────────────────────────────────────────────
      // No-Active-Child Auto-install Pause: post-safeguard-failure-first
      // window (T-INST-116k / T-INST-116k2)
      // SPEC §10.10 "Signals during the auto-install pass when no
      // npm child is active": the no-active-child contract must
      // hold for the gap between a safeguard failure being recorded
      // into the aggregate report's accumulator and the next
      // workflow's iteration beginning. The load-bearing assertion
      // is (f) "no aggregate failure report on stderr": even though
      // `failures` already accumulated the safeguard-failure entry
      // BEFORE the signal arrived, the end-of-pass suppression rule
      // ("Signal termination suppresses the final auto-install
      // aggregate failure report unless it had already been emitted")
      // still applies because the report has not yet been emitted.
      // The FIFO-replacement FAULT applied to all three workflows
      // makes the test order-independent: whichever workflow the
      // implementation processes first has a FIFO at its `.gitignore`
      // immediately before the safeguard `lstat`, and the safeguard
      // failure is recorded for that workflow alone.
      // T-INST-116k: SIGINT axis.
      // T-INST-116k2: SIGTERM axis (parity).
      // ─────────────────────────────────────────────────────────

      it("T-INST-116k: SIGINT during post-safeguard-failure-first window — no spawn, FIFO preserved, aggregate report suppressed", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm({ exitCode: 0, logFile }, async (fake) => {
          ...
          // would-be exit code 1 from the recorded safeguard-failure.
          expect(r.exitCode).toBe(130);

          // (b) Zero shim invocations — the failed workflow's safeguard
          // failure caused npm install to be skipped for it; the
          // notReached workflows were preempted by the signal before
          // their iterations began.
          const invocations = fake.readInvocations();
          expect(invocations.length).toBe(0);

          // (c) `.loopx/<failed>/.gitignore` is still a FIFO — loopx
          // did not unlink, replace, or otherwise touch the FIFO entry;
          // the safeguard's non-regular branch left it unchanged.
          ...
          expect(failedSt.isFIFO()).toBe(true);

          // (d) Workflows in notReached do NOT have a `.gitignore` on
          // disk — the FAULT seam fires immediately before each
          // workflow's safeguard `lstat`, so workflows whose iteration
          // never began did not have a FIFO placed.
          for (const w of notReached) {
            expect(existsSync(join(project!.loopxDir, w, ".gitignore"))).toBe(
              false,
            );
          ...
          // (e) All three committed workflow directories preserved
          // byte-for-byte from the source — committed workflow files
          // are not rolled back on safeguard failure or signal.
          for (const w of ["alpha", "beta", "gamma"]) {
            const indexPath = join(project!.loopxDir, w, "index.sh");
            const pkgPath = join(project!.loopxDir, w, "package.json");
            expect(existsSync(indexPath)).toBe(true);
          ...
          // `failures` already accumulated `failed`'s safeguard-failure
          // entry before the signal arrived. This is the load-bearing
          // assertion for the suppression-on-accumulator-state contract.
          expect(r.stderr).not.toMatch(/auto-install failures/);
        });
      });

      it("T-INST-116k2: SIGTERM during post-safeguard-failure-first window — SIGTERM-axis parity", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm({ exitCode: 0, logFile }, async (fake) => {
          ...
          // (c) FIFO at `.loopx/<failed>/.gitignore` preserved.
          const failedGitignore = join(
            project!.loopxDir,
            failed,
            ".gitignore",
          ...
          expect(existsSync(failedGitignore)).toBe(true);
          expect(lstatSync(failedGitignore).isFIFO()).toBe(true);
          ...
          // parity for the suppression-on-accumulator-state contract.
          // A buggy implementation that handled the suppression
          // correctly under SIGINT but mishandled it under SIGTERM
          // (e.g., via a separate signal-handler dispatch path) would
          // pass T-INST-116k and fail this assertion.
          expect(r.stderr).not.toMatch(/auto-install failures/);
        });
      });
      ...
      // ─────────────────────────────────────────────────────────
      // SIGINT/SIGTERM during the auto-install pass — no-active-child
      // before-first-workflow ordinal window
      // (T-INST-116n / T-INST-116n2)
      // SPEC §10.10 "Signals during the auto-install pass when no
      // npm child is active": the no-active-child contract must
      // hold even at the very top of the auto-install pass — BEFORE
      // the first workflow's iteration body begins. The marker
      // payload reports `processed = []` (no workflow's iteration
      // has started), `current = committed[0]` (upcoming first
      // workflow), and `remaining = committed.slice(1)` (the others).
      // The load-bearing observation: an implementation that wires
      // its no-active-child signal handling only into the
      // per-workflow loop body (e.g., one that checks for
      // `receivedSignal` before each workflow's safeguard but skips
      // the check on the very first iteration's pre-pass setup)
      // would pass T-INST-116h/116i/116j/116k/116m and fail this
      // test. With ZERO workflows processed and an EMPTY `failures`
      // accumulator, the aggregate report is structurally
      // unreachable and the suppression guard is the only thing
      // standing between the seam-injected pause and a clean exit.
      // T-INST-116n: SIGINT axis.
      // T-INST-116n2: SIGTERM axis (parity).
      // ─────────────────────────────────────────────────────────

      it("T-INST-116n: SIGINT during before-first-workflow window — zero workflows processed, no .gitignore on disk anywhere", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm({ exitCode: 0, logFile }, async (fake) => {
          ...
          // `processed` is empty: no workflow's iteration has started
          // when the seam fires.
          expect(marker.processed).toEqual([]);
          const upcoming = marker.current as string;
          expect(["alpha", "beta", "gamma"]).toContain(upcoming);
          ...
          for (const w of remaining) {
            expect(["alpha", "beta", "gamma"]).toContain(w);
            expect(w).not.toBe(upcoming);
          }
          const allWorkflows = [upcoming, ...remaining];
          expect(new Set(allWorkflows)).toEqual(
          ...
          // (b) Zero shim invocations — the head-of-loop signal-check
          // guard at the top of the per-workflow loop short-circuits
          // ALL workflows the moment SIGINT is observed, so npm
          // install is never spawned.
          const invocations = fake.readInvocations();
          expect(invocations.length).toBe(0);

          // (c) NO `.gitignore` exists on disk for any workflow —
          // the safeguard's synthesis side effect never ran for
          // any workflow because no per-workflow body executed.
          for (const w of ["alpha", "beta", "gamma"]) {
            expect(existsSync(join(project!.loopxDir, w, ".gitignore"))).toBe(
              false,
            );
          ...
          // (d) All three committed workflow directories preserved
          // byte-for-byte from the source — committed workflow
          // files are not rolled back under SPEC §10.10's
          // signal-termination clause.
          for (const w of ["alpha", "beta", "gamma"]) {
            const indexPath = join(project!.loopxDir, w, "index.sh");
            const pkgPath = join(project!.loopxDir, w, "package.json");
            expect(existsSync(indexPath)).toBe(true);
          ...
          // (e) No aggregate failure report on stderr. With
          // `failures` empty (no workflow's iteration ran), the
          // aggregate-report block is structurally unreachable; the
          // suppression guard at end-of-pass returns 0 before the
          // report could be emitted. A buggy implementation that
          // wired the dispatch into the loop body but skipped the
          // pre-loop check would still produce no aggregate report
          // here (because no failures were recorded), so this
          // assertion is necessary but not sufficient — the
          // load-bearing assertions are (b) zero shim invocations
          // and (c) no `.gitignore` on disk anywhere.
          expect(r.stderr).not.toMatch(/auto-install failures/);
        });
      });

      it("T-INST-116n2: SIGTERM during before-first-workflow window — SIGTERM-axis parity", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm({ exitCode: 0, logFile }, async (fake) => {
          ...
          // (b) Zero shim invocations — SIGTERM-axis parity for
          // the no-spawn guarantee at the before-first-workflow
          // window.
          const invocations = fake.readInvocations();
          expect(invocations.length).toBe(0);

          // (c) No `.gitignore` on disk anywhere.
          for (const w of ["alpha", "beta", "gamma"]) {
            expect(existsSync(join(project!.loopxDir, w, ".gitignore"))).toBe(
              false,
            );
          ...
          // (e) No aggregate failure report on stderr — SIGTERM-axis
          // parity for the no-aggregate-report guarantee. A buggy
          // implementation whose suppression rule was conditioned on
          // SIGINT specifically (rather than the SPEC's SIGINT-or-SIGTERM
          // normative pair) would pass T-INST-116n and fail this
          // assertion.
          expect(r.stderr).not.toMatch(/auto-install failures/);
        });
      });
      ...
      // ─────────────────────────────────────────────────────────
      // SIGINT/SIGTERM during the auto-install pass — no-active-child
      // post-aggregate-report ordinal window
      // (T-INST-116l / T-INST-116l2)
      // SPEC §10.10's signal-termination clause has two halves:
      // signal termination SUPPRESSES the final auto-install
      // aggregate failure report UNLESS it had already been emitted.
      // T-INST-116k pins down the SUPPRESSION half (failure
      // recorded, report not yet emitted, signal observed → report
      // suppressed). T-INST-116l covers the complementary
      // ALREADY-EMITTED CARVE-OUT half: signal arrives AFTER the
      // aggregate report has been written / flushed to stderr but
      // before the process exits. The report MUST remain on stderr
      // and MUST NOT be retroactively suppressed; committed workflow
      // files plus already-created auto-install side effects (the
      // FIFOs left by the gitignore-replace-with-fifo FAULT) MUST
      // remain on disk; and `npm install` MUST have been spawned
      // exactly zero times because every workflow's safeguard
      // failed.
      //
      // Together with T-INST-116k this closes both halves of SPEC
      // §10.10's signal-termination clause for the aggregate-report
      // path, with full no-rollback parity to the suppression-half
      // tests.
      //
      // T-INST-116l: SIGINT axis.
      // T-INST-116l2: SIGTERM axis (parity).
      // ─────────────────────────────────────────────────────────

      it("T-INST-116l: SIGINT during post-aggregate-report window — already-emitted report preserved, no rollback, zero spawns", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm({ exitCode: 0, logFile }, async (fake) => {
          ...
          // ONLY after the aggregate report has been emitted to
          // stderr (and after every qualifying workflow has reached
          // a terminal auto-install outcome — partial processing is
          // not conforming at this window).
          const pauseDeadline = Date.now() + 30_000;
          while (!existsSync(markerPath)) {
            if (Date.now() > pauseDeadline) {
              sendSignal("SIGINT");
          ...
          // Per TEST-SPEC §1.4: per-workflow focal-position fields
          // do not apply at this window. `current` is null,
          // `processed` is the full set of workflow names whose
          // processing reached a terminal outcome, and `remaining`
          // is the empty array.
          ...
          // Pin (e) at marker-read time so a buggy implementation
          // that emitted the report (and fired the seam) prematurely
          // with `remaining` non-empty would fail here, before the
          // signal is even sent. (TEST-SPEC: "a buggy implementation
          // that paused before all qualifying workflows were
          // processed would surface a non-empty `remaining` array,
          // failing the section 1.4 contract for the
          // post-aggregate-report ordinal".)
          ...
          // (a) Exit 130 (128 + SIGINT) — the signal supersedes the
          // would-be exit code 1 from the recorded aggregate failure
          // per SPEC §10.10's "128 + signal number" clause.
          expect(r.exitCode).toBe(130);

          // (b) The aggregate failure report IS present on stderr —
          // the report was written before the pause and the signal
          // cannot un-emit it. This is the load-bearing carve-out
          // assertion: a buggy implementation that retroactively
          // suppressed the already-emitted report on signal would
          // pass T-INST-116k (suppression half) and fail this test.
          expect(r.stderr).toMatch(/Error: auto-install failures:/);
          // The report shape per-workflow: each of alpha/beta/gamma
          // has a safeguard-failure entry (FIFO replacement → "not
          // a regular file" branch).
          expect(r.stderr).toMatch(/\[alpha\].*not a regular file/);
          expect(r.stderr).toMatch(/\[beta\].*not a regular file/);
          expect(r.stderr).toMatch(/\[gamma\].*not a regular file/);

          // (c) NO second copy of the aggregate report appears in
          // stderr — the carve-out preserves the already-emitted
          // report; it does not re-emit. A buggy implementation
          // that re-wrote the report after the signal observation
          // would surface two report headers here.
          const headerMatches = r.stderr.match(
            /Error: auto-install failures:/g,
          );
          expect(headerMatches?.length ?? 0).toBe(1);
          ...
          // file-level install. Parity with the no-rollback
          // contract that T-INST-116h / 116i / 116j / 116k assert.
          for (const w of ["alpha", "beta", "gamma"]) {
            const indexPath = join(project!.loopxDir, w, "index.sh");
            const pkgPath = join(project!.loopxDir, w, "package.json");
            expect(existsSync(indexPath)).toBe(true);
          ...
          // (e) `processed` equals exactly {alpha, beta, gamma}
          // (set equality, order not asserted — implementation's
          // auto-install order is implementation-defined per
          // SPEC §10.10). Already validated at marker-read time
          // above; re-asserting here keeps the assertion list
          // aligned with TEST-SPEC §3 T-INST-116l.
          expect(new Set(processed)).toEqual(
            new Set(["alpha", "beta", "gamma"]),
          );

          // (f) For each workflow, the .gitignore FIFO that the
          // FAULT seam materialized before that workflow's
          // safeguard `lstat` is still on disk as a FIFO. The
          // auto-install side effects already produced before the
          // signal observation are preserved per SPEC §10.10's
          // "side effects completed before the signal observation
          // remain on disk" rule.
          for (const w of ["alpha", "beta", "gamma"]) {
            const gitignorePath = join(project!.loopxDir, w, ".gitignore");
            expect(existsSync(gitignorePath)).toBe(true);
            expect(lstatSync(gitignorePath).isFIFO()).toBe(true);
          }

          // (g) The fake-npm shim log records exactly zero
          // invocations — `npm install` was never spawned for any
          // workflow because each workflow's `.gitignore` safeguard
          // failed under the FIFO-replacement FAULT and SPEC §10.10
          // specifies "`npm install` is skipped for that workflow"
          // on safeguard failure. Without (g), an implementation
          // that recorded each safeguard failure into the
          // aggregate accumulator AND still spawned `npm install`
          // afterward would pass (a)–(f) — the aggregate report
          // would still list every workflow's safeguard failure,
          // the FIFOs would still be on disk because npm wouldn't
          // write `.gitignore`, and committed files would still be
          // intact.
          const invocations = fake.readInvocations();
          expect(invocations.length).toBe(0);
        });
      ...
      it("T-INST-116l2: SIGTERM during post-aggregate-report window — SIGTERM-axis parity", async () => {
        project = await createTempProject();
        const logFile = join(project.dir, "fake-npm.log");
        const markerPath = join(project.dir, "autoinstall-pause-marker.json");
        gitServer = await startLocalGitServer([
        ...
        await withFakeNpm({ exitCode: 0, logFile }, async (fake) => {
          ...
          // (b) Aggregate report present on stderr — SIGTERM-axis
          // parity for the carve-out half. A buggy implementation
          // could mishandle this under SIGTERM via an over-zealous
          // signal cleanup path that retroactively wiped already-
          // flushed stderr buffers (in environments where stderr is
          // buffered) or retroactively rolled back committed
          // workflow files.
          expect(r.stderr).toMatch(/Error: auto-install failures:/);
          expect(r.stderr).toMatch(/\[alpha\].*not a regular file/);
          expect(r.stderr).toMatch(/\[beta\].*not a regular file/);
          ...
          // (e) processed contains all three workflow names.
          expect(new Set(processed)).toEqual(
            new Set(["alpha", "beta", "gamma"]),
          );

          // (f) FIFO side effects preserved.
          for (const w of ["alpha", "beta", "gamma"]) {
            const gitignorePath = join(project!.loopxDir, w, ".gitignore");
            expect(existsSync(gitignorePath)).toBe(true);
            expect(lstatSync(gitignorePath).isFIFO()).toBe(true);
          ...
          // (g) Zero shim invocations — SIGTERM-axis parity for
          // the SPEC §10.10 "skipped" rule.
          const invocations = fake.readInvocations();
          expect(invocations.length).toBe(0);
        });
      }, { timeout: 60_000, retry: 2 });
